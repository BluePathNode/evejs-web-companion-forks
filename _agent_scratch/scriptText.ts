// A3 â€” every sentence a player reads about a VALID program, in one place.
//
// âš  THIS IS THE R9a REGISTER FOR THE PROGRAM VIEW. The codec owns the sentences
// for a REJECTED file (scriptCodec's SAY); this owns the sentences for a good
// one â€” a step, a condition, a repeat, an interrupt â€” so the editor and the run
// readout speak with one voice and a test can sweep them all.
//
// âš  R7d LIVES HERE TOO. Nothing in this file renders a numeric id. A world slot
// shows the NAME the reference carries, and when it has none it says "a station
// you pick" â€” never the number. The exhaustiveness switches carry no `default`,
...
/** A percentage a player reads â€” always with its unit, never a bare number. */
function pct(fraction: number): string {
  return `${Math.round(fraction * 100)}%`;
}

/**
 * An ISK amount a player reads â€” grouped with its unit. Grouping is pinned to
...
/** A world reference as words â€” its name, or a "you pick" phrase; never its id (R7d). */
...
export function macroName(macro: MacroID): string {
  switch (macro) {
    ...
    case "deliver-ready-jobs":
      return "Collect finished industry jobs";
    case "repair-ship":
    ...
    case "join-fleet":
      return "Join a fleet";
    case "join-fleet-from-advert":
      return "Join a fleet from advert";
    case "standing-fleet-boss":
      return "Run as Standing Fleet Boss";
    case "attack-player":
    ...
    case "deploy-mtu-and-loot":
      return "Deploy a Mobile Tractor Unit and loot";
    case "wait-while-asteroids-above":
      return "Warp nearest belt and wait while asteroids above";
    case "empty-belt-cleanup-next":
      return "Empty-belt cleanup and next belt";
    case "tractor-loot-jetcans":
      return "Tractor jetcans into the ore hold";
    case "empty-belt-home-tether-wait":
      return "Empty belt then home tether and wait";
...
/** A named board slot, as a player reads it â€” never a key, never an id. */
...
/** The words an "alert me" watch shows when it fires â€” a whole sentence a player
...
function macroPhrase(step: MacroStep): string {
  switch (step.macro) {
    ...
    case "mine-at-belt": {
      ...
      const focus = step.args["focus"];
      const focusPhrase =
        focus !== undefined && focus.kind === "rockFocus" ? `, focusing on ${focus.name}` : "";
      return `Mine at ${where}${order}${focusPhrase}`;
    }
    case "travel-to-station": {
    ...
    case "deliver-ready-jobs":
      return "Collect every finished industry job";
    case "repair-ship":
    ...
    case "join-fleet-from-advert": {
      ...
      const name =
        nameArg !== undefined && nameArg.kind === "text" && nameArg.text.trim().length > 0
          ? nameArg.text.trim()
          : null;
      return name !== null
        ? `Apply to join a fleet whose advert name contains ${name}`
        : "Apply to join the first available fleet advert";
    }
    case "standing-fleet-boss": {
      const nameArg = step.args["advertName"];
      const name =
        nameArg !== undefined && nameArg.kind === "text" && nameArg.text.trim().length > 0
          ? nameArg.text.trim()
          : "your fleet";
      const by = step.args["acceptBy"];
      const mode =
        by !== undefined && by.kind === "fleetAcceptBy"
          ? by.acceptBy
          : "corp";
      const standing = step.args["minStandings"];
      const threshold =
        standing !== undefined && standing.kind === "standing" ? standing.value : 0;
      if (mode === "corp") {
        return `Form a fleet, advertise as ${name}, and auto-accept corporation applicants`;
      }
      if (mode === "alliance") {
        return `Form a fleet, advertise as ${name}, and auto-accept alliance applicants`;
      }
      return `Form a fleet, advertise as ${name}, and auto-accept applicants with standings greater than ${threshold}`;
    }
    case "attack-player": {
    ...
    case "deploy-mtu-and-loot":
      return "While stationary in an industrial support ship, deploy a Mobile Tractor Unit if needed, then loot the unit and any jetcans already within range";
    case "wait-while-asteroids-above": {
      const gate = step.args["greaterThan"];
      const n = gate !== undefined && gate.kind === "count" ? gate.value : 0;
      return (
        `Warp to the nearest belt, then wait while asteroid count is greater than ${n}` +
        ` (done when count drops to ${n} or below)`
      );
    }
    case "empty-belt-cleanup-next": {
      ...
      const mode =
        modeArg !== undefined && modeArg.kind === "beltTravel" && modeArg.mode === "random"
          ...
          : "the nearest";
      return (
        `When the belt is empty: scoop Mobile Tractor Units if possible, recall drones, ` +
        `then warp to ${mode} other belt`
      );
    }
    case "tractor-loot-jetcans":
      return "Activate fitted tractor beams on jetcans and loot ore into the ore hold";
    case "empty-belt-home-tether-wait":
      return "When the belt is empty: recall drones, travel to your bot home (starting structure), undock to tether, then wait";
    case "fleet-warp-to-me": {
      const gate = step.args["greaterThan"];
