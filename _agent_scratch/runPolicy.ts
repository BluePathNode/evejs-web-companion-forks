...
export const MACRO_RUN_POLICY: Readonly<Record<MacroID, MacroRunPolicy>> = Object.freeze({
  ...
  // Delivery only ever gives; re-reading jobs confirms. Idempotent collect - safe to resume.
  "deliver-ready-jobs": policy(["inventory"]),
  ...
  "join-fleet-from-advert": policy(["fleet"]),
  ...
  "deploy-mtu-and-loot": policy(["inventory"]),
  "wait-while-asteroids-above": SAFE,
  "empty-belt-cleanup-next": policy(["inventory"]),
  "tractor-loot-jetcans": policy(["inventory"]),
  "empty-belt-home-tether-wait": SAFE,
