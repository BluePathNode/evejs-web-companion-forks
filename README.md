# EveJS Web Companion Fork

## Fork

This is a **parallel fork** of the EveJS web companion for custom macros/UI work.
It lives outside the EveJS mods tree so upstream under
Eve Offline\...\mods\evejs-web-companion-master stays untouched.

- **Upstream / credit:** EveJS web companion (evejs-web-poc) — original browser companion for EveJS.
- **This fork:** evejs-web-companion-fork — same stack, separate install and config.
- **Ports:** this fork uses **:26501**; upstream companion uses **:26500**.
- **EveJS root:** points at the existing EveJS checkout (native mode) so both can talk to the same Docker EveJS server.
- **Changelog:** fork-specific history lives in [CHANGELOG.md](CHANGELOG.md) (living doc; keep appending under Unreleased).
...
## Upwell / structure docking (fork)

See [docs/FORK_UPWELL.md](docs/FORK_UPWELL.md) for bot home / haul / autopilot support when the dock is an Upwell structure rather than an NPC station.
