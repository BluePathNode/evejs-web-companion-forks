<script lang="ts">
    ...
    if (macro === "standing-fleet-boss") {
      return {
        ...
        args: {
          ...
          advertDescription: { kind: "text", text: "" },
          acceptBy: { kind: "fleetAcceptBy", acceptBy: "corp" },
          minStandings: { kind: "standing", value: DEFAULT_STANDING_ARG },
        },
      };
    }
  ...
  /** The mine block's ore-family focus — absent means any mineable rock. */
  function rockFocusValue(step: MacroStep): string {
    const arg = step.args["focus"];
    return arg !== undefined && arg.kind === "rockFocus" ? arg.name : "";
  }
  function setStepRockFocus(i: number, raw: string, side: Side | null = null, j = -1): void {
    updateStep(
      i,
      (s) => {
        if (raw === "") {
          const { focus: _dropped, ...rest } = s.args;
          return { ...s, args: rest };
        }
        return {
          ...s,
          args: {
            ...s.args,
            focus: { kind: "rockFocus", name: raw as (typeof ROCK_FOCUS_NAMES)[number] },
          },
        };
      },
      side,
  ...
  function fleetAcceptByValue(step: MacroStep): string {
    const arg = step.args["acceptBy"];
    return arg !== undefined && arg.kind === "fleetAcceptBy" ? arg.acceptBy : "corp";
  }
  function setStepFleetAcceptBy(i: number, raw: string, side: Side | null = null, j = -1): void {
    const acceptBy = (FLEET_ACCEPT_BY_ARGS as readonly string[]).includes(raw) ? (raw as "corp" | "alliance" | "standings") : "corp";
    updateStep(i, (s) => ({ ...s, args: { ...s.args, acceptBy: { kind: "fleetAcceptBy", acceptBy } } }), side, j);
  }
  function beltTravelValue(step: MacroStep): string {
    const arg = step.args["mode"];
    return arg !== undefined && arg.kind === "beltTravel" ? arg.mode : "nearest";
  }
  function setStepBeltTravel(i: number, raw: string, side: Side | null = null, j = -1): void {
    updateStep(
      i,
      (s) => {
        if (raw !== "random") {
          const { mode: _dropped, ...rest } = s.args;
          return { ...s, args: rest };
        }
        return { ...s, args: { ...s.args, mode: { kind: "beltTravel", mode: "random" } } };
      },
      side,
      j,
    );
  }
  function standingArgValue(step: MacroStep, key: string): number {
    const arg = step.args[key];
    return arg !== undefined && arg.kind === "standing" ? arg.value : DEFAULT_STANDING_ARG;
  }
  function setStepStandingArg(i: number, key: string, raw: number, side: Side | null = null, j = -1): void {
    const value = Math.min(MAX_STANDING_ARG, Math.max(MIN_STANDING_ARG, Math.trunc(Number(raw) || 0)));
    updateStep(i, (s) => ({ ...s, args: { ...s.args, [key]: { kind: "standing", value } } }), side, j);
  }
  ...
  {#snippet macroEditors(step: MacroStep, i: number, side: "then" | "else" | null, j: number)}
          ...
          {#if step.macro === "mine-at-belt"}
            <span class="inline-edit">
              ...
              <select value={rockFocusValue(step)} onchange={(e) => setStepRockFocus(i, e.currentTarget.value, side, j)}>
                <option value="">rock first</option>
                {#each ROCK_FOCUS_NAMES as oreName}
                  <option value={oreName}>{oreName} asteroids</option>
                {/each}
              </select>
              at
              <select
          ...
          {/if}
                    {#if step.macro === "wait-while-asteroids-above"}
            <span class="inline-edit">
              wait while asteroids &gt;
              <input class="pct" type="number" min="0" max="500" placeholder="0" value={countArgValue(step, "greaterThan") ?? ""} oninput={(e) => setStepCountArg(i, "greaterThan", e.currentTarget.value, 0, 500, side, j)} />
              (blank = 0)
            </span>
          {/if}
          {#if step.macro === "empty-belt-cleanup-next"}
            <span class="inline-edit">
              then warp to
              <select value={beltTravelValue(step)} onchange={(e) => setStepBeltTravel(i, e.currentTarget.value, side, j)}>
...
{#if step.macro === "jettison-cargo"}
          ...
          {/if}
          {#if step.macro === "standing-fleet-boss"}
            <span class="inline-edit">
              advert
              <input class="chat-in" type="text" maxlength={MAX_TEXT_ARG_LEN} placeholder="fleet name…" value={textArgValue(step, "advertName")} oninput={(e) => setStepTextArg(i, "advertName", e.currentTarget.value, side, j)} />
              —
              <input class="chat-in" type="text" maxlength={MAX_TEXT_ARG_LEN} placeholder="description…" value={textArgValue(step, "advertDescription")} oninput={(e) => setStepTextArg(i, "advertDescription", e.currentTarget.value, side, j)} />
              accept by
              <select value={fleetAcceptByValue(step)} onchange={(e) => setStepFleetAcceptBy(i, e.currentTarget.value, side, j)}>
                <option value="corp">corporation</option>
                <option value="alliance">alliance</option>
                <option value="standings">standings greater than</option>
              </select>
              {#if fleetAcceptByValue(step) === "standings"}
                <input class="pct" type="number" min={MIN_STANDING_ARG} max={MAX_STANDING_ARG} value={standingArgValue(step, "minStandings")} oninput={(e) => setStepStandingArg(i, "minStandings", Number(e.currentTarget.value), side, j)} />
              {/if}
            </span>
          {/if}
  {/snippet}
...
<style>
  ...
  .side-row {
    ...
    border-radius: var(--radius-frame);
    border: 1px solid color-mix(in srgb, var(--color-line) 55%, transparent);
  ...
  .empty {
    color: var(--color-muted);
    background: var(--color-panel-3);
    border: 1px dashed color-mix(in srgb, var(--color-line) 80%, transparent);
    border-radius: var(--radius-frame);
    padding: 0.8rem;
    text-align: center;
  }
