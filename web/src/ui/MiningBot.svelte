<script lang="ts">
  ...
  /** Somewhere to put the ore: a station/structure in view, plus where you docked. */
  const stations = $derived.by<Choice[]>(() => {
    const rows = (snapshot?.entities ?? [])
      .filter((entity) => (entity.kind === "station" || entity.kind === "structure") && !entity.isSelf)
      .map((entity) => ({
        id: entity.itemID,
        label: entity.name ?? (entity.kind === "structure" ? "Structure" : "Station"),
        distance: formatDistance(surfaceTo(entity)),
      }));
    ...
    // Upwell home: StationStatic may be null (NPC-station tables only), so also
    // offer the flight-status dock location (stationID ?? structureID).
    const flightHomeID = $flight.status?.docked
      ? ($flight.status.stationID ?? $flight.status.structureID)
      : null;
    if (
      flightHomeID !== null &&
      flightHomeID > 0 &&
      !rows.some((row) => row.id === flightHomeID)
    ) {
      const label =
        $flight.stationName ??
        $flight.structureName ??
        ($flight.status?.structureID === flightHomeID ? "Structure you docked at" : "Station you docked at");
      rows.push({ id: flightHomeID, label, distance: "where you docked" });
    }
