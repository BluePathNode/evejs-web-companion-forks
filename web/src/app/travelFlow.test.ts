...
function defaultResponder(path: string): { status: number; body: unknown } {
  ...
  if (path.startsWith("/api/map/resolve/")) {
    ...
    if (id === 1030000000001) {
      return { status: 200, body: { ok: true, id, kind: "structure", stationID: id, stationName: "Home Citadel", solarSystemID: 3, systemName: "Charlie", lookupFailed: false } };
    }
    if (id === 99999999) {
...
test("startRoute to an Upwell structure keeps a dock destination (does not arrive undocked)", async () => {
  const store = createClientStore();
  const flow = createAppFlow(store, { fetch: makeFakeFetch(defaultResponder) });

  const outcome = await flow.startRoute(1030000000001);
  flow.abortRoute();

  assert.equal(outcome.started, true);
  const travel = store.travel.get();
  assert.equal(travel.destinationSystemID, 3);
  assert.equal(travel.destinationStationID, 1030000000001, "structure id is the dock target");
  assert.equal(travel.destinationName, "Home Citadel");
  assert.equal(travel.totalJumps, 2);
});
