...
test("the roster marks exactly ONE pilot as foreground", () => {
  ...
  // that reason. During Add Character / restore, joining is true and every
  // roster pilot is quiesced (no push, no foreground) so SelectCharacterID
  // can finish.
  ...
  assert.match(effect, /setQuiesced\(joining\)/, "roster must quiesce while a pilot is joining");
  assert.match(
    effect,
    /setLivePush\(!joining && isActive && tabMayHoldLivePush\)/,
    "push is no longer driven by the active id (and must stay off while joining)",
  );
  assert.match(
    effect,
    /setForeground\(!joining && isActive\)/,
    "the roster never marks a foreground pilot (and must yield while joining)",
  );
});

// --- In-tab join: quiesce frees lanes for SelectCharacterID -----------------
//
// Add Character / restore keep the active Workspace mounted. Without quiesce,
// 2 Hz space polls + bot ticks + SelectCharacterID share MAX_IN_FLIGHT=4 and
// the EveJS edge owner, which is the real ~3-in-one-tab drop.

test("setQuiesced closes the live stream; un-quiesce reattaches when push is on", async () => {
  const { flow, store, streams } = await loginAndSelect(false);
  flow.setLivePush(true);
  assert.equal(streams.opened.length, 1);

  flow.setQuiesced(true);
  assert.equal(streams.closed, 1);
  assert.equal(store.live.get().status, "idle");
  // Idempotent.
  flow.setQuiesced(true);
  ...
  flow.setQuiesced(false);
  assert.equal(streams.opened.length, 2);
});

test("setQuiesced does not reopen push when livePush is off", async () => {
  const { flow, streams } = await loginAndSelect(false);
  flow.setQuiesced(true);
  flow.setQuiesced(false);
  assert.equal(streams.opened.length, 0);
});
