...
/** Raw /api/bridge/fleet-ads envelope (available + my advert). */
...
/** APPLY to join a public fleet-finder advert by fleetID. Confirm-gated. */
export async function applyToJoinFleet(
  fleetID: number,
  autoAccept: boolean = true,
  options: ApiOptions = {},
): Promise<void> {
  await postJson("/api/bridge/fleet/apply", { fleetID, autoAccept, confirm: true }, options);
}

/** POST a fleet-finder advert for the session fleet (must be boss). Confirm-gated. */
/** Send a fleet location/history broadcast (SendBroadcast). Confirm-gated. */
export async function sendFleetBroadcast(
  args: {
    readonly name: string;
  ...
  await postJson("/api/bridge/fleet/broadcast", { ...args, confirm: true }, options);
}

/** Fork: last remembered fleet location broadcast for this session's fleet. */
export async function loadLastFleetBroadcast(
  options: ApiOptions = {},
): Promise<Record<string, JsonValue>> {
  return getJson("/api/bridge/fleet/last-broadcast", options);
}

export async function addFleetAdvert(
  advertData: Readonly<Record<string, unknown>>,
  options: ApiOptions = {},
): Promise<void> {
  await postJson("/api/bridge/fleet/advert/add", { advertData, confirm: true }, options);
}
...
/**
 * Launch deployables from ship cargo (ship.LaunchFromShip) — Mobile Tractor Units,
 * mobile depots, etc. Confirm-gated on the BFF. Success is judged by the next
 * space/cargo re-read, not by trusting the ack alone.
 */
export async function launchFromShip(
  itemIDs: readonly number[],
  options: ApiOptions = {},
): Promise<void> {
  await postJson("/api/bridge/ship/launch-from-ship", { itemIDs: [...itemIDs], confirm: true }, options);
}
