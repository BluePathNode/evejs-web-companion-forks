// The Bot Builder document â€” the shape a player-authored bot is SAVED, IMPORTED
// and EXPORTED in, plus the pure structural helpers that read it.
//
// âš  THIS FILE IS THE FORMAT AND NOTHING ELSE. It holds no player-facing strings
...
// truth for what a valid document can CONTAIN â€” the codec validates against these
// types and these bounds, the editor builds them, the runner reads them.
//
// â”€â”€â”€ WHY THE SHAPE IS WHAT IT IS â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
//
...
// list of STEPS run top to bottom. Two safety properties are STRUCTURAL here â€”
// unrepresentable rather than validated â€” so no import can strip them:
//
//   â€¢ A CYCLE IS UNDRAWABLE. Order is an array index and a step has no outgoing
...
//   â€¢ TWO CALLS IN ONE TICK IS UNREPRESENTABLE. A step names exactly one macro;
...
// â”€â”€â”€ Format identity â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€â”€ Shared bounds â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
//
// âš  ONE SOURCE OF TRUTH FOR EVERY LIMIT. The codec clamps to these, the editor
 ...
 * fraction with headroom to spare â€” so a player cannot ask for 100% and reopen
 * that bug.
 */
export const MAX_ORE_HOLD_FRACTION = 0.9;

/** Health/shield thresholds run 0.05..0.95 â€” never 0 (an interrupt that can never fire). */
export const MIN_CONDITION_FRACTION = 0.05;
export const MAX_CONDITION_FRACTION = 0.95;

/** Structural size caps â€” the shape of the document, independent of byte length. */
...
// â”€â”€â”€ World references â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
 ...
 *
 * âš  THE ID IS A HINT, NEVER TRUSTED. It makes a same-world re-import exact, but
 * an imported document lands on another character's world where the same number
 * may name a different thing â€” so on import every ref is UNBOUND until the id
...
export interface WorldRef {
  ...
  /**
   * STATION ONLY. True means "wherever the ship started" â€” resolved at run time
  ...
  /**
   * STATION ONLY. A NAMED BOARD SLOT â€” "the station an earlier block found",
...
/** A "return to where you started" station ref â€” the portable default. */
export function startingStation(): WorldRef {
  return { entity: "station", id: null, name: null, systemName: null, starting: true };
}

// â”€â”€â”€ Macro arguments â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

/**
 * Where to mine. "nearest" is bound at RUNTIME from a fresh snapshot â€” it is the
...
/**
 * Which fitted modules are the miners â€” identified by their type GROUP, never a
 * name regex (R47: 17482 â†’ "Strip Miner"). `label` is the display hint.
 */
...
/**
 * An ISK amount a player sets â€” a buy ceiling, a sell floor, a wallet threshold.
...
/** A market quantity (units to buy) â€” past the small-count cap, still exact. */
...
/** Who the Standing Fleet Boss advert accepts — EveJS inviteScope corp / alliance / public. */
export type FleetAcceptByArg = "corp" | "alliance" | "standings";
export const FLEET_ACCEPT_BY_ARGS: readonly FleetAcceptByArg[] = Object.freeze<FleetAcceptByArg[]>([
  "corp",
  "alliance",
  "standings",
]);

/** Next-belt pick for empty-belt cleanup: nearest unemptied, or random among them. */
export type BeltTravelArg = "nearest" | "random";
export const BELT_TRAVEL_ARGS: readonly BeltTravelArg[] = Object.freeze<BeltTravelArg[]>([
  "nearest",
  "random",
]);

/** Standing threshold for fleet-finder auto-accept (strictly greater than). */
export const MIN_STANDING_ARG = -10;
export const MAX_STANDING_ARG = 10;
export const DEFAULT_STANDING_ARG = 0;


/** The hunt block's editable defaults â€” shared by the editor and the runtime. */
...
export type Arg =
  ...
  /** A specific agent (WorldRef entity "agent"). Optional on mission blocks â€” left
   * unset, the block uses the agent the find block published on the run's board. */
  | { readonly kind: "agent"; readonly ref: WorldRef }
  /** A bounded small integer: an agent LEVEL (1â€“5) or a max-jumps ceiling. */
  ...
  /** An ISK amount the player sets â€” a buy ceiling or a sell floor (per unit). */
  | { readonly kind: "isk"; readonly value: number }
  /** A market quantity â€” how many units a buy order is for. */
  ...
  /** A chat channel to talk in â€” a closed vocabulary, validated by the codec. */
  ...
  /**
   * Which asteroid/ore family a mining step focuses on. Absent = any mineable
   * rock (shipped behaviour). Matched case-insensitively as a substring of the
   * rock's overview name (so "Mercoxit" also hits "Magma Mercoxit").
   */
  | { readonly kind: "rockFocus"; readonly name: RockFocusName }
  ...
  /** How empty-belt-cleanup-next chooses the next belt (default nearest). */
  | { readonly kind: "beltTravel"; readonly mode: BeltTravelArg }
  /** A short line of text the player writes (a chat message). Never empty at run
   * time â€” the validator flags a blank one before the bot can start. */
 ...
 *
 *   â€¢ "nearest" â€” the shipped behaviour, and still the default: least flying.
 *   â€¢ "biggest" â€” the most ore left first, from the amount the snapshot already
 ...
 *     than being treated as empty â€” a null is not a zero.
 */
export type RockPick = "nearest" | "biggest";
export const ROCK_PICKS: readonly RockPick[] = Object.freeze<RockPick[]>(["nearest", "biggest"]);

/**
 * Ore-family names the mine block can focus on. Closed vocabulary so the Bot
 * Builder can offer a dropdown; matching is a case-insensitive substring of the
 * rock's name, so variants (Concentrated Veldspar, Magma Mercoxit, ...) still hit.
 */
export const ROCK_FOCUS_NAMES = Object.freeze([
  "Veldspar",
  "Scordite",
  "Pyroxeres",
  "Plagioclase",
...
export type RockFocusName = (typeof ROCK_FOCUS_NAMES)[number];

/** True when a rock's overview name belongs to the focused ore family. */
export function rockNameMatchesFocus(
  rockName: string | null | undefined,
  focusName: string,
): boolean {
  if (rockName === null || rockName === undefined || rockName.length === 0) return false;
  if (focusName.length === 0) return false;
  return rockName.toLowerCase().includes(focusName.toLowerCase());
}
 ...
 * (met / not-met / cannot-tell) from that tick's FRESH reads â€” never the store â€”
 * with cannot-tell never passing. Here it is only the shape + its threshold.
 *
 * âš  `hostile-on-grid` IS A GRID READ. It is false-or-unknowable while the ship
 ...
 * trivially true mid-warp â€” the belt-empty-on-tick-one trap in another costume.
...
export type Condition =
   ...
   * â€” for a hauler, a looter, a salvager, anything whose hold is not ore.
   */
  ...
  /** A player's ship on this grid has THIS ship locked â€” you are being hunted. */
...
/** Every condition kind â€” for exhaustive iteration in menus and tests. */
...
/**
 * The legal sites for a condition kind â€” the structural guard on the belt-empty
...
export function conditionSites(kind: ConditionKind): readonly ConditionSite[] {
  // âš  EVERY GRID / SURROUNDINGS READ IS INTERRUPT-ONLY, for the reason spelled out
  ...
  //   â€¢ hostile-on-grid / targeted-by-player / drone-health-below â€” grid reads.
  //   â€¢ players-in-system-above â€” an awareness watch on who else is here; it is a
...
// â”€â”€â”€ Interrupts ("Always watching") â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

/**
 * What a fired interrupt does.
 *
 *   â€¢ "pause"          â€” stop where you are and say why.
 *   â€¢ "dock-and-pause" â€” break off, dock at home, pause (the safety-floor
 *                        response, and the hostile "run for the station" pick).
 *   â€¢ "launch-drones"  â€” put drones out and KEEP WORKING (the hostile "use
 *                        drones" pick). Bounded by the existing three-attempt
 *                        launch rule, which heads home if it cannot.
 */
/**
 *   â€¢ "repair"         â€” switch the matching repairers ON while the condition
 *                        holds and the capacitor allows, and back OFF when it
 *                        clears â€” the "keep the ship repaired" watch. The step
 *                        keeps working; repairs ride the watching layer.
 *   â€¢ "alert"          â€” TELL THE PLAYER and keep working: a notification, a
 ...
 *
 * âš  TWO PROPERTIES MAKE "alert" BEHAVE, and both live in the orchestrator
 * (nav/scriptDecide), not here:
 *   â€¢ IT FIRES ONCE PER EPISODE. A condition that stays met would otherwise
 *     alert every tick â€” thirty notifications a minute, which is how an alert
 ...
 *   â€¢ A SPENT ALERT ROW IS TRANSPARENT. Interrupts are first-match-wins, so an
 *     alert row sitting above a dock-and-pause row would silence it forever.
 *     Once spent, the scan skips the row and carries on down the ladder â€” so
 *     "tell me, AND dock" is two rows that both work.
 */
export type InterruptResponse = "pause" | "dock-and-pause" | "launch-drones" | "repair" | "alert";

/** Every interrupt response â€” for exhaustive iteration in menus and tests. */
...
// â”€â”€â”€ Program nodes â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

/**
 * Which macros the format knows. The first five are the mining set; the seven
 * `*-mission` / agent blocks are the DISTRIBUTION-MISSION set â€” each one step of
 ...
 * into blocks a player wires up â€” usually inside a Repeat loop.
 */
export type MacroID =
  | "undock"
  | "travel-to-station"
  ...
  | "deliver-ready-jobs"
  | "repair-ship"
  // â”€â”€ The market set. Place orders at the station's market (server confirm-gated).
  | "buy-item"
  | "sell-item"
  // â”€â”€ The fleet-support set. Remote-repair friendly ships on grid (logistics).
  | "remote-rep"
  | "orbit-and-boost"
  // â”€â”€ The fleet-management set. Form up / invite / join (multibox alt-fleeting).
  | "create-fleet"
  | "invite-to-fleet"
  | "join-fleet"
  | "standing-fleet-boss"
  | "join-fleet-from-advert"
  // â”€â”€ The PvP set. Camp a grid / roam and hunt another player's ship.
  | "attack-player"
  | "hunt-player"
  // â”€â”€ Social. Say something in a chat channel (pairs with a branch for
  //    "announce when a check holds").
  | "send-chat"
  // â”€â”€ Movement extras. Point the autopilot somewhere; run for the nearest dock.
  | "set-destination"
  | "dock-at-nearest"
  // â”€â”€ Fleet support extra: feed a mate's capacitor.
  | "remote-cap"
  // â”€â”€ Cargo extras: dump a can into space; tidy the hangar.
  | "jettison-cargo"
  | "jettison-ore"
  | "tidy-hangar"
  // â”€â”€ Mining extra: squeeze the ore down against a support ship on grid.
  | "compress-ore"
  // Industrial support (Orca / Porpoise / Rorqual): deploy MTU + loot.
  | "deploy-mtu-and-loot"
  ...
  | "industrial-core-on"
  | "industrial-core-off"
  // â”€â”€ Exploration: a safe probe sweep driven by EveJS's current authority.
  | "launch-scan-probes"
  | "analyze-signatures"
  | "recover-scan-probes";

/** Every macro id â€” for exhaustive iteration in menus and tests. */
...
/**
 * One macro step. `until` is the player's DO-UNTIL â€” the step runs until the
 ...
 * each pass â€” "mine; if the hold is full, haul home, else keep going"). A loop
 ...
 * place the program forks â€” and it stays cycle-free: both sides are forward-only
 * `MacroStep` lists (no loops, no nested branches â€” one level, like a loop body),
...
/**
 * "Run one of my other saved bots here" â€” composition without copy-paste.
 *
 * âš  IT IS EXPANDED (INLINED) BEFORE THE RUN STARTS, never resolved mid-run: the
 ...
 * The included bot's OWN watches and home are ignored â€” the bot you start
 * governs the run.
 */
...
// â”€â”€â”€ The document â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

/**
 * A whole player bot, as saved / imported / exported.
 *
 * `home` is required â€” every bot names the station it docks at when a
...
// â”€â”€â”€ Structural helpers â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
//
// Pure reads over a document. They never throw â€” a readout or validator must not
...
/** Both sides of a branch as one list â€” the steps it can run. */
 ...
 * A sub-bot counts as ONE here â€” it is replaced by its real steps at expansion,
 ...
 * `MAX_TOTAL_STEPS` cap bounds â€” a loop of 3 steps is 3, and a branch of 2-then
 * + 1-else is 3, not 1.
 */
...
/**
 * The macro step with this id, wherever it sits â€” top level or inside a loop
 * body â€” or null. Ids are unique within a document (the codec enforces it), so
