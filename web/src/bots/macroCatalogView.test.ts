...
test("undock needs nothing; mine-at-belt needs a belt, equipment, and an until", () => {
  ...
  assert.deepEqual(mine.params.map((p) => p.key).sort(), ["belt", "equipment", "focus", "pick"]);
  ...
  assert.equal(mine.params.find((p) => p.key === "focus")?.required, false, "the ore-family focus is optional (any rock)");
