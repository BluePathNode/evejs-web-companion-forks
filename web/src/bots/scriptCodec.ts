...
function readArg(raw: unknown, expected: Arg["kind"], label: string, ctx: Ctx): Arg {
  ...
  if (expected === "rockFocus") {
    const name = obj["name"];
    if (typeof name !== "string" || !ROCK_FOCUS_NAMES.includes(name as RockFocusName)) {
      refuse(SAY.badArg(label));
    }
    return { kind: "rockFocus", name: name as RockFocusName };
  }
  ...
  if (expected === "fleetAcceptBy") {
    const acceptBy = obj["acceptBy"];
    if (typeof acceptBy !== "string" || !FLEET_ACCEPT_BY_ARGS.includes(acceptBy as FleetAcceptByArg)) {
      refuse(SAY.badArg(label));
    }
    return { kind: "fleetAcceptBy", acceptBy: acceptBy as FleetAcceptByArg };
  }
  if (expected === "beltTravel") {
    const mode = obj["mode"];
    if (typeof mode !== "string" || !BELT_TRAVEL_ARGS.includes(mode as BeltTravelArg)) {
      refuse(SAY.badArg(label));
    }
    return { kind: "beltTravel", mode: mode as BeltTravelArg };
  }
  if (expected === "standing") {
    ...
    const clamped = Math.min(MAX_STANDING_ARG, Math.max(MIN_STANDING_ARG, n));
    if (clamped !== n) {
      ctx.warn(WARN.clampCount(label, clamped));
    }
    return { kind: "standing", value: clamped };
  }
  if (expected === "text") {
...
function orderArg(arg: Arg): unknown {
  switch (arg.kind) {
    ...
    case "fleetAcceptBy":
      return { kind: "fleetAcceptBy", acceptBy: arg.acceptBy };
    case "beltTravel":
      return { kind: "beltTravel", mode: arg.mode };
    case "standing":
      return { kind: "standing", value: arg.value };
    case "text":
    ...
    case "rockFocus":
      return { kind: "rockFocus", name: arg.name };
    default: {
