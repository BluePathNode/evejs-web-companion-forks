...
test("rockNameMatchesFocus: case-insensitive substring; empty never matches", () => {
  assert.equal(rockNameMatchesFocus("Mercoxit", "Mercoxit"), true);
  assert.equal(rockNameMatchesFocus("Magma Mercoxit", "mercoxit"), true);
  assert.equal(rockNameMatchesFocus("Veldspar", "Mercoxit"), false);
  assert.equal(rockNameMatchesFocus(null, "Mercoxit"), false);
  assert.equal(rockNameMatchesFocus("Mercoxit", ""), false);
  assert.ok(ROCK_FOCUS_NAMES.includes("Mercoxit"));
});
