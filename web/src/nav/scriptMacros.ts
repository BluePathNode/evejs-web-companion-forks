// B1 â€” the macro adapters. Each block is an independent "task": it decides ONE
...
// activate, unload, launch/engage drones). No new ship control â€” the block model
...
import { BOARD_SLOT_KEY, DEFAULT_HUNT_MAX_JUMPS, DEFAULT_HUNT_RANGE_AU, DEFAULT_STANDING_ARG, rockNameMatchesFocus } from "../bots/botScript.ts";
import {
  advertMatchesConfig,
  buildStandingFleetAdvertData,
  joinRequestMatchesAcceptBy,
  type FleetAcceptByMode,
...
const ORBIT_RANGE_M = 5000; // the operator's "orbit of 5km" â€” inside mining range
...
// SALVAGE_RANGE_M and ORBIT_BOOST_RANGE_M/REMOTE_ASSIST_RANGE_M below) â€” the
...
/** Consecutive Activate tries for one miner that never appears active. */
const MINING_ACTIVATE_ATTEMPTS = 5;
...
function textArg(step: MacroStep, key: string): string | null {
  const arg = step.args[key];
  if (arg === undefined || arg.kind !== "text") return null;
 ...
 * and hold there until 0 drones are left in space â€” then return null so the caller
...
function recallBeforeLeaving(
  ...
  // 0 drones still in space â†’ nothing to wait for; the caller may warp out.
 ...
 * "chosen" pins one belt by NAME (never id â€” unlike a station, a belt's id is
 ...
 *
 * âš  `remainingQuantity` IS NULL FOR UNKNOWN, NEVER ZERO (the snapshot is explicit
...
// â”€â”€ undock â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
/** Optional ore-family focus from the step; null = any mineable rock. */
function rockFocusName(step: MacroStep): string | null {
  const arg = step.args["focus"];
  if (arg === undefined || arg.kind !== "rockFocus") return null;
  return arg.name;
}

/** Mineable rocks on grid, narrowed by the step's optional focus. */
function focusedRocks(step: MacroStep, entities: readonly SpaceEntity[]): SpaceEntity[] {
  const mineable = entities.filter((entity) => isMineableRock(entity));
  const focus = rockFocusName(step);
  if (focus === null) return mineable;
  return mineable.filter((rock) => rockNameMatchesFocus(rock.name, focus));
}
...
// â”€â”€ travel-to-belt â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
// Warp to a belt â€” a pinned one, or the nearest â€” and stop once on grid with
...
const travelToBelt: MacroDecider = (step, obs) => {
  ...
  if (obs.inWarp === true) {
    return tick(WAIT, "In warp â€” nothing decided mid-warp.", "Flying to the belt", ACTING, false, {});
  }
...
// â”€â”€ mine-at-belt â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
// Find the nearest rock, orbit it at 5km, lock it, run the miners, and when it
// is gone pick the next â€” warping to the step's belt (a pinned one, or else
...
const mineAtBelt: MacroDecider = (step, obs, mem) => {
  const snapshot = obs.snapshot ?? null;
  if (obs.inWarp === true) {
    return tick(WAIT, "In warp â€” nothing decided mid-warp.", "Flying to the belt", ACTING, false, mem);
  }
  if (obs.inSpace !== true || snapshot === null) {
  ...
  const measurement = measureSpace(snapshot);
  const rocks = focusedRocks(step, snapshot.entities);
  ...
  if (rocks.length === 0) {
    ...
    const beltDist = measurement?.distances.get(belt.itemID) ?? Number.POSITIVE_INFINITY;
    if (beltDist <= BELT_ARRIVAL_RADIUS_M) {
      return tick(WAIT, (() => { const focus = rockFocusName(step); return focus ? `At the belt, but no ${focus} asteroids left.` : "At the belt, but there are no rocks left."; })(), "Belt empty", {
        kind: "blocked",
        reason: (() => {
          const focus = rockFocusName(step);
          return focus ? `No ${focus} asteroids left on this belt.` : "This belt has no rocks left to mine.";
        })(),
      });
    }
    return tick({ kind: "warp", targetID: belt.itemID }, `Warping to ${rockLabel(belt)}.`, "Flying to the belt", ACTING, false, {});
  ...
  if (!locked) {
    ...
    if (waited > MAX_LOCK_WAIT_TICKS) {
      // It will not lock â€” let it go and pick another next tick.
  ...
  // Locked, but a lock reaches much further than the mining equipment does â€” a
  ...
  if (outOfRange) {
    if (num(mem, "approachedRockID") !== rockID) {
      return tick(
        ...
        "Closing in â€” too far out to mine yet.",
        "Approaching a rock",
        ACTING,
        true,
    ...
    return tick(WAIT, "Closing in â€” too far out to mine yet.", "Approaching a rock", ACTING, true, {
      rockID,
      lockIssued: true,
      waited: 0,
  ...
  // Locked and in range â€” switch on any mining module that is not already cycling.
  //
  // âš  AN EMPTY LIST IS NOT "NOTHING LEFT TO SWITCH ON". `miningModuleIDs` is
  // auto-detected from the ship's fit (flow.ts's `resolveMiningModuleIDs`, by
  // high-slot + the game's own mining group names) â€” if that comes back empty
  ...
  // null activeModuleIDs is UNKNOWN â€” never "everything is off". Re-Activating
  // because the ship would not say is exactly the fabricated progress the
  // mining bot loop refuses to make, and on a noisy snapshot it is also how
  // modules thrash on/off every tick.
  const activeIDs = snapshot.ship?.activeModuleIDs ?? null;
  if (activeIDs === null) {
    return tick(WAIT, "Your ship did not say which equipment is running, so nothing was switched on.", "Mining", ACTING, true, {
      rockID, lockIssued: true, waited: 0, approachedRockID: rockID,
    });
  }
  ...
  if (nextMiner !== undefined) {
    // Bound per-module: a laser that never appears in activeModuleIDs must not
    // be re-fired forever (cap starvation / silent refusal). Same ceiling the
    // dedicated mining bot uses.
    const prevModule = num(mem, "activateModuleID");
    const prevAttempts = num(mem, "activateAttempts") ?? 0;
    const attempts = prevModule === nextMiner ? prevAttempts + 1 : 1;
    if (attempts > MINING_ACTIVATE_ATTEMPTS) {
      return tick(WAIT, "Mining equipment would not stay on.", "Mining", {
        kind: "blocked",
        reason: "Mining equipment was switched on several times and the ship still reports it as off, so the bot stopped rather than pretend it is mining.",
      });
    }
    return tick(
      ...
      ACTING,
      true,
      { rockID, lockIssued: true, waited: 0, approachedRockID: rockID, activateModuleID: nextMiner, activateAttempts: attempts },
    );
  }
  return tick(WAIT, "Mining the rock.", "Mining", ACTING, true, {
    rockID, lockIssued: true, waited: 0, approachedRockID: rockID, activateModuleID: null, activateAttempts: 0,
  });
};

// â”€â”€ deliver-ore â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
// Fly to the station â€” same system or across the map, on the SHARED autopilot â€”
...
const deliverOre: MacroDecider = (step, obs, mem, board) => {
  ...
  if (obs.flightStatus?.docked === true && dockedLocationID(obs.flightStatus) === target) {
  ...
  // Heading out to haul â€” never warp off with drones still out. Align only to a
...
// â”€â”€ travel-to-station â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
// Fly to ANY station and dock â€” same system or across the map. The trip rides
...
// â”€â”€ defend-with-drones â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
const defendWithDrones: MacroDecider = (_step, obs, mem) => {
  ...
  if (target === null) {
    // The pirates are gone. Bring the drones home BEFORE finishing â€” the block
    ...
    if (myDrones.length > 0) {
      return tick({ kind: "recallDrones", droneIDs: myDrones }, "The pirates are gone â€” calling the drones back in.", "Recalling drones", ACTING);
    }
...
// â•â•â• The distribution-mission set â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
function rideAutopilotTo(obs: ScriptObservation, stationID: number, phase: string): MacroTick | null {
  if (obs.flightStatus?.docked === true && dockedLocationID(obs.flightStatus) === stationID) {
    return null; // arrived
  }
  const travel = obs.travel ?? null;
  // Only a failure on THIS destination blocks â€” a stale reason left over from an
  ...
  if (travel !== null && travel.status === "running" && travel.destinationStationID === stationID) {
    return tick(WAIT, "Flying there â€” the autopilot has the ship.", phase, ACTING, false);
  }
  return tick({ kind: "startRoute", stationID }, "Setting the destination and heading out.", phase, ACTING, false);
}

// â”€â”€ find-*-agent â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ request-mission â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ accept-mission â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
// Gate the offer (cargo fits, trip length), then Accept â€” or Decline and ask
...
function unreadableOffer(
  ...
  if (briefing === null || briefing === undefined) {
    return null; // no offer on the table at all â€” the caller asks for one
  }
...
const acceptMission: MacroDecider = (step, obs, mem, board) => {
  ...
  // âš  A READING WE DO NOT HAVE IS NOT A REASON TO TURN A JOB DOWN. Declining is
  // an irreversible act against the agent â€” it burns the offer and starts a
  // decline timer â€” so it must never be the answer to "I could not see".
  //
  ...
  if (briefing !== null) {
    ...
    // âš  NO CEILING MEANS THE ROUTE DOES NOT MATTER. `gateOffer` turns a job down
    // when it has no jump count at all â€” right when a limit is in play, wrong
  ...
  // No offer on the table â€” ask for one (covers the after-a-decline case).
  const requestID = agentActionID(conversation, AGENT_BUTTON.REQUEST_MISSION);
  if (requestID !== null) {
    return tick(
      { kind: "agentButton", agentID, actionID: requestID, label: "Request Mission" },
...
// â”€â”€ load-mission-cargo â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
// Move the package from the pickup hangar into the ship, and confirm it is
// really aboard before calling itself done â€” never trusting the 200.
const loadMissionCargo: MacroDecider = (_step, obs, mem, board) => {
  ...
  if (typeID === null || quantity === null) {
    return tick(WAIT, "No mission cargo to load.", "Loading cargo", {
      kind: "blocked",
      reason: "There is no accepted mission naming cargo to load â€” accept one first.",
    });
  }
...
// â”€â”€ travel-to-dropoff â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ turn-in-mission â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ wait â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
// A plain delay: do nothing for N seconds (default 10), counted in ticks off the
// runner's own cadence â€” no clock reads, so it is pure and testable. ARMED from
// tick one, so a player's `until` ("stop when shields are back over 80%") can end
// it early â€” which turns the same block into "wait for X".
...
const waitBlock: MacroDecider = (step, _obs, mem) => {
  ...
  return tick(WAIT, `Waiting â€” about ${remaining} seconds left.`, "Waiting", ACTING, true, { ticked });
};

// â”€â”€ unload-cargo â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
// Docked: move EVERYTHING in the cargo hold into the station hangar. Done only
// when a fresh cargo read shows the hold empty â€” a 200 is not an empty hold.
const unloadCargo: MacroDecider = (_step, obs, mem) => {
  if (obs.flightStatus?.docked !== true) {
    return tick(WAIT, "Not docked, so there is no hangar to unload into.", "Emptying the hold", {
      kind: "blocked",
      reason: "Dock at a station first â€” this block empties your cargo into its hangar.",
    });
  }
...
// â”€â”€ return-to-agent â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ salvage-wrecks / loot-wrecks â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
/** A wreck that is YOURS to open â€” owned by you or your corp. Unknown owner = NO. */
function isOwnWreck(wreck: SpaceEntity, obs: ScriptObservation): boolean {
  ...
  if (wreck.ownerID === null) {
    return false; // cannot tell whose it is â€” never open it
  }
...
// nearest wreck through the approachâ†’lockâ†’activate ladder. Done (drones home)
// when no wrecks remain. Salvaging any wreck is legal â€” only LOOTING is gated.
const salvageWrecks: MacroDecider = (_step, obs, mem) => {
  const snapshot = obs.snapshot ?? null;
  if (obs.inWarp === true) {
    return tick(WAIT, "In warp â€” nothing decided mid-warp.", "Salvaging", ACTING, false, mem);
  }
  if (obs.inSpace !== true || snapshot === null) {
  ...
  if (wrecks.length === 0) {
    // Swept clean â€” bring the drones home before finishing (never abandon them).
    if (myDrones.length > 0) {
      return tick({ kind: "recallDrones", droneIDs: myDrones }, "All salvaged â€” calling the drones home.", "Salvaging", ACTING);
    }
  ...
  // The module ladder, one wreck at a time â€” the mine block's shape on a wreck.
  const measurement = measureSpace(snapshot);
  let wreckID = num(mem, "wreckID");
  if (wreckID !== null && !wrecks.some((w) => w.itemID === wreckID)) {
    wreckID = null; // it salvaged away â€” pick the next
  }
  if (wreckID === null) {
  ...
  if (!locked) {
    ...
    const waited = (num(mem, "waited") ?? 0) + 1;
    if (waited > MAX_LOCK_WAIT_TICKS) {
      return tick(WAIT, "That wreck would not lock â€” moving on.", "Salvaging", ACTING, true, { ...mem, wreckID: null });
    }
...
// next. A wreck whose owner cannot be read is NEVER opened â€” that is the whole
// "no can flipping" rule, structural rather than polite.
const lootWrecks: MacroDecider = (_step, obs, mem) => {
  const snapshot = obs.snapshot ?? null;
  if (obs.inWarp === true) {
    return tick(WAIT, "In warp â€” nothing decided mid-warp.", "Looting", ACTING, false, mem);
  }
  if (obs.inSpace !== true || snapshot === null) {
...
// â”€â”€ loot-containers â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// How long to keep checking an apparently-empty grid before believing it â€”
...
// it, next. No ownership check â€” this is an emulator, not a client guarding
// real players from can-flipping, and the server enforces none either.
//
// âš  NOT MARKED DONE THE INSTANT IT IS ISSUED. flow.ts's lootContainer dispatch
// re-reads the transfer's own result, but nothing here ever saw it â€” so
...
// instant the SERVER sees it truly empty (any looter, not just this one â€” see
// jettisonRuntime.js's maybeExpireEmptySpaceContainer) â€” so "still on grid" IS
...
const lootContainers: MacroDecider = (_step, obs, mem) => {
  ...
  if (cans.length === 0) {
    ...
    // grid never had one â€” landing on a belt and checking for containers on
  ...
  // A can IS present this tick â€” the settle window above was for nothing, and
...
// â”€â”€ refine-ore â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
// Docked: refine every ore stack in the station hangar. Ore is the game's own
// Asteroid category (25) on the row â€” a row whose category CANNOT be read is
...
const CATEGORY_ORE = 25; // retail's Asteroid category â€” every ore type lives in it

const refineOre: MacroDecider = (_step, obs, mem) => {
  if (obs.flightStatus?.docked !== true) {
    return tick(WAIT, "Not docked, so there is no refinery to use.", "Refining", {
      kind: "blocked",
      reason: "Dock at a station first â€” this block refines the ore in its hangar.",
    });
  }
...
// â”€â”€ hardeners-on â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
const hardenersOn: MacroDecider = (_step, obs, mem) => {
  ...
  if (activeIDs === null) {
    return tick(WAIT, "Your ship did not say which equipment is running, so nothing was switched on.", "Hardening", ACTING, false, mem);
  }
  const active = new Set(activeIDs);
...
// â”€â”€ fight-the-rats â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
// The full combat loop: nearest pirate first â€” lock it (bounded), set the drones
...
// Players on grid are FRIENDLY in this world (operator decision) â€” only NPC
// hostiles (hostileRows) are ever engaged.
const fightTheRats: MacroDecider = (_step, obs, mem) => {
  const snapshot = obs.snapshot ?? null;
  if (obs.inWarp === true) {
    return tick(WAIT, "In warp â€” nothing decided mid-warp.", "Fighting", ACTING, false, mem);
  }
  if (obs.inSpace !== true || snapshot === null) {
  ...
  if (hostiles.length === 0) {
    if (myDrones.length > 0) {
      return tick({ kind: "recallDrones", droneIDs: myDrones }, "Grid clear â€” calling the drones home.", "Fighting", ACTING);
    }
  ...
  // Drones out first â€” they defend on their own the moment they undock.
  ...
  if (!locked) {
    ...
    const waited = (num(mem, "waited") ?? 0) + 1;
    if (waited > MAX_LOCK_WAIT_TICKS) {
      return tick(WAIT, "That one would not lock â€” picking another.", "Fighting", ACTING, true, {});
    }
...
// â”€â”€ warp-to-anomaly â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
const warpToAnomaly: MacroDecider = (_step, obs, mem, board) => {
  if (obs.flightStatus?.docked === true) {
    return tick(WAIT, "Docked â€” there is no scanner to fly on from here.", "Scanning", {
      kind: "blocked",
      reason: "Undock first â€” put a Leave-the-station block before this one.",
    });
  }
  if (flag(mem, "issued")) {
    if (obs.inWarp === true) {
...
// â”€â”€ refit-ship â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
const refitShip: MacroDecider = (step, obs, mem) => {
  if (obs.flightStatus?.docked !== true) {
    return tick(WAIT, "Not docked â€” refitting happens in a station.", "Refitting", {
      kind: "blocked",
      reason: "Dock at a station first â€” refitting happens in its hangar.",
    });
  }
...
// â”€â”€ move-items â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
// The generic logistics block, docked only: move a picked ITEM between the
// hangar, the cargo hold and the ore hold â€” a set amount (split off one stack
...
// â”€â”€ warp-to-bookmark â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
// Warp to a saved spot, matched BY NAME from the live bookmark list (the id is a
// same-world hint). In-space only, and only in the spot's own system â€” a saved
...
// â”€â”€ fly-to-mission-site â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ restart-extractors â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
// Walk every colony and restart each extractor whose program has EXPIRED â€” on
...
const restartExtractors: MacroDecider = (_step, obs, mem) => {
  ...
  for (const colony of colonies) {
    for (const pin of colony.extractors) {
      ...
      if (!expired && !neverRan) {
        continue; // still running â€” leave it be
      }
      if (pin.resourceTypeID === null) {
        skippedUnknown += 1;
        continue; // no last resource to reuse â€” never guess what to pull
      }
      const attempts = (num(mem, "attempts") ?? 0) + 1;
...
// â”€â”€ deliver-ready-jobs â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
// Walk the industry job list and DELIVER each job whose status is "ready" â€”
// the SERVER's own ready derivation (never a client clock vs endDate). One
// deliver per tick, confirmed by the next jobs re-read (the job leaves the
// ready set). Delivery only ever gives, so this runs from anywhere.
const deliverReadyJobs: MacroDecider = (_step, obs, mem) => {
  const jobs = obs.industryJobs ?? null;
  if (jobs === null) {
    return tick(WAIT, "Reading your industry jobs.", "Collecting jobs", ACTING, false, mem);
  }
  const ready = jobs.filter((job) => job.status === "ready");
  if (ready.length === 0) {
    return tick(WAIT, "No finished industry jobs to collect.", "Collecting jobs", { kind: "done" });
  }
  const attempts = (num(mem, "attempts") ?? 0) + 1;
  if (attempts > MAX_BLOCK_ATTEMPTS * 4) {
    return tick(WAIT, "The deliveries kept not landing.", "Collecting jobs", {
      kind: "blocked",
      reason: "The industry deliveries kept not taking, so the bot stopped.",
    });
  }
  const job = ready[0]!;
  return tick(
    { kind: "deliverIndustryJob", jobID: job.jobID },
    ready.length === 1
      ? "Collecting the finished industry job."
      : `Collecting a finished industry job (${ready.length} ready).`,
    "Collecting jobs",
    ACTING,
    false,
    { attempts },
...
// â”€â”€ repair-ship â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ buy-item â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
// Docked: place ONE buy order at this station's market for the picked item, at
// the player's price ceiling, for the quantity they set. One-shot â€” it places the
...
// â”€â”€ sell-item â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â•â•â• The fleet-support set â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
/**
 * How far remote assistance reaches â€” remote reps and cap transmitters alike.
 *
 * âš  DELIBERATELY THE SMALL END, with margin, in the style of `SALVAGE_RANGE_M`.
 ...
 *
 * âš  THIS BOUND WAS MISSING ENTIRELY. Both remote blocks used to return `mem`
 * UNCHANGED after issuing an activate, so a refused module was re-found idle on
 * the next tick and re-fired â€” forever, with no counter, no progress and no
 ...
 * Returns the tick to emit, or null when there is nobody to rep right now â€” the
...
function repHurtMate(
  ...
  // (once per target â€” `approach` is a standing follow order, not a nudge), and
  ...
  if (outOfReach && mayApproach && num(mem, "repApproached") !== target.itemID) {
    return tick(
      { kind: "approach", targetID: target.itemID },
      "Closing in â€” too far out for the reps to reach.",
      phase,
      ACTING,
      true,
  ...
  const active = new Set(snapshot.ship?.activeModuleIDs ?? []);
  // âš  CONSECUTIVE failures only. A rep that HAS come on refills the budget, so
  // the bound can only ever stop a module that is not landing â€” never one that is
...
// â”€â”€ remote-rep â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ orbit-and-boost â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
const orbitAndBoost: MacroDecider = (_step, obs, mem) => {
  ...
  // No approach from in here â€” the orbit above is this block's way of closing.
...
// â•â•â• The fleet-management set â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
// Form up / invite / join â€” the multibox alt-fleeting loop. All confirm-gated
...
// never a blank). âš  the WRITES were never fired live â€” flagged for QA.

// â”€â”€ create-fleet â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ invite-to-fleet â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ join-fleet â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// List session-visible fleet-finder adverts, match optional advertName substring,
...
const joinFleetFromAdvert: MacroDecider = (step, obs, mem) => {
  ...
  const advertName =
    nameArg !== undefined && nameArg.kind === "text" ? nameArg.text : "";
  const nameFilter = normalizeAdvertNameFilter(advertName);
  const label = "Joining from advert";

  const inFleet = obs.inFleet ?? null;
  if (inFleet === true) {
    return tick(WAIT, "You are in a fleet now.", label, { kind: "done" });
  }
  if (inFleet === null) {
    return tick(WAIT, "Checking your fleet status.", label, ACTING, false, mem);
  }

  const ads = obs.availableFleetAds;
  if (ads === null || ads === undefined) {
    return tick(WAIT, "Looking up fleet adverts.", label, ACTING, false, mem);
  }

  const match = pickMatchingFleetAdvert(ads, { advertName: nameFilter });
  if (match === null || match.fleetID === null) {
    const reason =
      nameFilter.length > 0
        ? `No fleet advert matching "${nameFilter}" is visible to you.`
        : "No fleet adverts are visible to you.";
    return tick(WAIT, reason, label, {
      kind: "blocked",
      reason: reason + " Ask the boss to post a Standing Fleet Boss advert open to you.",
    });
  }

  const fleetID = match.fleetID;
  const waited = (num(mem, "waited") ?? 0) + 1;
  if (waited > JOIN_ADVERT_MAX_WAIT_TICKS) {
    return tick(WAIT, "Joining from the advert timed out.", label, {
      kind: "blocked",
      reason:
...
// ── standing-fleet-boss ──────────────────────────────────────────────────────
// Composing boss loop: form fleet → post fleet-finder advert → keep accepting.
// Corp/alliance auto-accept is enforced by EveJS inviteScope. Standings stores
// public_minStanding on the advert (EveJS does not yet gate ApplyToJoinFleet on
// it). Pending join requests that match the rule are invited as a belt-and-
// suspenders path. Sustained while the step runs (never self-dones).
const standingFleetBoss: MacroDecider = (step, obs, mem) => {
  const nameArg = step.args["advertName"];
  const descArg = step.args["advertDescription"];
  const byArg = step.args["acceptBy"];
  if (nameArg === undefined || nameArg.kind !== "text" || nameArg.text.trim().length === 0) {
    return tick(WAIT, "No advert name set.", "Standing Fleet Boss", {
      kind: "blocked",
      reason: "Write a name for the fleet advert.",
    });
  }
  if (descArg === undefined || descArg.kind !== "text" || descArg.text.trim().length === 0) {
    return tick(WAIT, "No advert description set.", "Standing Fleet Boss", {
      kind: "blocked",
      reason: "Write a description for the fleet advert.",
    });
  }
  if (byArg === undefined || byArg.kind !== "fleetAcceptBy") {
    return tick(WAIT, "No accept rule set.", "Standing Fleet Boss", {
      kind: "blocked",
      reason: "Pick whether to accept by corporation, alliance, or standings.",
    });
  }
  const acceptBy = byArg.acceptBy as FleetAcceptByMode;
  const standingArg = step.args["minStandings"];
  const minStandings =
    standingArg !== undefined && standingArg.kind === "standing"
      ? standingArg.value
      : DEFAULT_STANDING_ARG;
  const input = {
    advertName: nameArg.text,
    advertDescription: descArg.text,
    acceptBy,
    minStandings,
  };
  ...
  const inFleet = obs.inFleet ?? null;
  if (inFleet === null) {
    return tick(WAIT, "Checking your fleet status.", "Standing Fleet Boss", ACTING, false, mem);
  }
  if (inFleet === false) {
    const tries = (num(mem, "formTries") ?? 0) + 1;
    if (tries > MAX_BLOCK_ATTEMPTS) {
      return tick(WAIT, "The fleet would not form.", "Standing Fleet Boss", {
        kind: "blocked",
        reason: "The fleet would not form after several tries, so the bot stopped.",
      });
    }
    return tick(
      { kind: "createFleet" },
      "Forming a fleet so you can advertise it.",
      "Standing Fleet Boss",
      ACTING,
      false,
      { ...mem, formTries: tries },
  ...
  if (obs.fleetAdvertLive === null) {
    return tick(WAIT, "Checking the fleet advert.", "Standing Fleet Boss", ACTING, false, mem);
  }
  const advertOk = advertMatchesConfig(obs.fleetAdvert ?? null, input);
  if (!advertOk) {
    const tries = (num(mem, "advertTries") ?? 0) + 1;
    if (tries > MAX_BLOCK_ATTEMPTS) {
      return tick(WAIT, "The fleet advert would not post.", "Standing Fleet Boss", {
        kind: "blocked",
        reason: "The fleet advert would not post after several tries, so the bot stopped.",
      });
    }
    const advertData = buildStandingFleetAdvertData(input);
    return tick(
      { kind: "postFleetAdvert", advertData: advertData as unknown as Readonly<Record<string, unknown>> },
      `Posting fleet advert "${advertData.fleetName}".`,
      "Standing Fleet Boss",
      ACTING,
      false,
      { ...mem, formTries: 0, advertTries: tries },
    );
  }

  const requests = obs.fleetJoinRequests;
  if (requests !== null && requests !== undefined) {
    const bossCorp = obs.fleetAdvert?.leaderCorpID ?? obs.myCorporationID ?? null;
    const bossAlliance = obs.fleetAdvert?.leaderAllianceID ?? null;
    const invited = new Set(
      Array.isArray(mem.invitedCharIDs) ? (mem.invitedCharIDs as number[]) : [],
    );
    for (const req of requests) {
      if (!joinRequestMatchesAcceptBy(req, acceptBy, bossCorp, bossAlliance, minStandings)) {
        continue;
      }
      const charID = req.charID;
      if (charID === null || invited.has(charID)) continue;
      return tick(
        { kind: "inviteToFleet", charID },
        "Accepting a matching fleet application.",
        "Standing Fleet Boss",
        ACTING,
        false,
        { ...mem, advertTries: 0, invitedCharIDs: [...invited, charID] },
      );
    }
  }
  ...
  const modeLabel =
    acceptBy === "corp"
      ...
      : acceptBy === "alliance"
        ? "alliance mates"
        : `standings greater than ${minStandings}`;
  return tick(
    WAIT,
    `Fleet advert is live — auto-accepting ${modeLabel}.`,
    "Standing Fleet Boss",
    ACTING,
    false,
    { ...mem, advertTries: 0 },
...
// â•â•â• The PvP set â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
// TARGET PICK is new â€” a player's hull instead of an NPC's. `otherPlayerShipsOnGrid`
...
/** Player ships on grid that the filter allows â€” the block's prey. */
 ...
 *
 * âš  THIS BOUND IS THE WHOLE REASON TACKLE IS SAFE TO PUT BEFORE THE GUNS. A point
 ...
 * up in `activeModuleIDs` â€” so "activate the idle point, else shoot" would pick
 ...
 * actually reaches.
 *
 * âš  BOTH NUMBERS ARE MEASURED, NOT GUESSED. Live, 2026-07-25: at 17.9 km the
 ...
 * than the guns do, so "locked" never meant "in reach" â€” it just looked like it,
 ...
 * both â€” a disruptor must not be held back on the chance the fitted module is a
 ...
 * prevent.
 *
 * âš  THIS IS WHAT KEEPS THE TACKLE BUDGET HONEST, and the live run that earned it
 * is worth the paragraph. The bot undocked, closed on its target, said "Guns on
 * them" â€” and the target was neither pointed nor webbed, and warped away
...
/**
 * The shared PvP engage: nearest allowed player first â€” lock it (bounded), tackle
 * it (bounded), drones onto it, every idle gun onto it â€” concentrating fire
...
function engagePrey(
  ...
  // Drones out first â€” they defend and add damage the moment they undock.
  if (obs.dronesOut !== true && bay.length > 0) {
  ...
  if (targetID !== null && !prey.some((p) => p.itemID === targetID)) {
    targetID = null; // it died or left the grid â€” next
  }
  if (targetID === null) {
  ...
  if (!locked) {
    ...
    const waited = (num(mem, "waited") ?? 0) + 1;
    if (waited > MAX_LOCK_WAIT_TICKS) {
      return tick(WAIT, "That ship would not lock â€” picking again.", phase, ACTING, true, {});
    }
  ...
  //
  // âš  ONCE PER TARGET IS THE WHOLE BOUND, and it is enough: `approach` is a
  ...
  // is already burning would be a no-op at best, and â€” since only one action
  // fires per tick â€” would starve the guns for the whole approach. `approached`
  ...
  if (
    ...
    return tick(
      ...
      "Closing in â€” too far out for the web and the guns.",
      phase,
      ACTING,
      true,
  ...
  // TACKLE FIRST â€” hold them still before anything else. Bounded: after
  // MAX_TACKLE_ATTEMPTS ticks of a module that will not come on â€” refused for a
  // reason the server does not say out loud â€” the engage stops asking and
  // shoots, so a stubborn module can never cost the whole fight.
  //
  // âš  THE POINT AND THE WEB EACH GET THEIR OWN BUDGET, and that separation is
  ...
  // attempt â€” that keeps each budget for real mysteries instead of spending it on
...
// â”€â”€ attack-player â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
// The CAMP block: park it somewhere (compose movement blocks before it) and it
// attacks any player ship that lands on the grid â€” or one pilot alone. Sustained
...
const attackPlayer: MacroDecider = (step, obs, mem) => {
  if (obs.flightStatus?.docked === true) {
    return tick(WAIT, "Docked â€” camping happens out in space.", "Camping", {
      kind: "blocked",
      reason: "Undock first â€” put a Leave-the-station block before this one.",
    });
  }
  if (obs.inWarp === true) {
    return tick(WAIT, "In warp â€” nothing decided mid-warp.", "Camping", ACTING, false, mem);
  }
  if (obs.inSpace !== true || obs.snapshot == null) {
...
// â”€â”€ hunt-player â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
//   â€¢ prey on grid â†’ engage it (the same shared PvP core);
//   â€¢ another pilot in LOCAL â†’ sweep the directional scanner, warp down each
//     hit that is not already on grid, and move to a fresh vantage point (a
//     belt or a gate) to re-sweep when the hits run out;
//   â€¢ local empty â†’ roam: ride the shared autopilot one system over, picked at
...
const huntPlayer: MacroDecider = (step, obs, mem, board) => {
  if (obs.flightStatus?.docked === true) {
    return tick(WAIT, "Docked â€” hunting happens out in space.", "Hunting", {
      kind: "blocked",
      reason: "Undock first â€” put a Leave-the-station block before this one.",
    });
  }
  ...
  // Mark home ONCE, on the board â€” the flow reads it for the map and the
  ...
  // Prey on this grid beats everything â€” engage with fresh combat memory.
  const prey = preyOnGrid(snapshot, only);
  if (prey.length > 0) {
    // Carry ONLY the combat keys into the engage (the search keys would confuse
    // it), and carry ALL of them â€” every bound and every latch included, or they
    ...
    // the same way. âš  ANY new key engagePrey remembers has to be added here too.
    const combatKeys = {
  ...
  if (flag(mem, "chaseIssued")) {
    ...
    if (flag(mem, "chaseSawWarp")) {
      ...
      if (waited > HUNT_CHASE_WARP_WAIT_TICKS) {
        // The warp never began (a refused or unreachable hit) â€” skip that hit.
        visited = chaseID !== null ? [...visited, String(chaseID)] : visited;
        carried = { visitedHits: visited.join(","), vantageID: mem["vantageID"] };
      } else {
  ...
  if (candidates.length > 0) {
    ...
    if (hits === null) {
      return tick(WAIT, `${who} is in this system â€” sweeping the scanner.`, "Hunting", ACTING, false, carried);
    }
    ...
    if (next !== undefined) {
      return tick(
        ...
        `Warping down a scanner hit â€” ${who} is in this system.`,
        "Hunting",
        ACTING,
        false,
    ...
    // Fresh visited list â€” a new vantage sees the system from somewhere new.
    return tick(
  ...
  // Local is empty â€” ROAM. One system over at random, never past the leash.
  const travel = obs.travel ?? null;
  ...
  // âš  LEAVING THE SYSTEM DROPS EVERYTHING SCOPED TO IT, and `visitedHits` is the
  // one that matters. It holds the itemIDs of scanner hits already chased, and it
  // used to ride along on every jump â€” so a hunt that came back to a system it had
  ...
  // reason â€” "a new vantage sees the system from somewhere new" â€” so a whole new
  ...
  return tick(
    ...
    "Nobody around â€” roaming to the next system.",
    "Hunting",
    ACTING,
    true,
...
// â”€â”€ send-chat â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
// Say ONE line in local or corp chat, then move on. Chat sends have no readable
// echo to confirm against, so this is deliberately one-shot: issue once, done â€”
...
// â•â•â• Movement extras â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•

// â”€â”€ set-destination â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// A system destination lands in space (no dock) â€” the autopilot's own
...
const setDestination: MacroDecider = (step, obs, mem) => {
  ...
  // Under way (or already arrived) on THIS destination â€” the block's job is done.
...
// â”€â”€ dock-at-nearest â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// block â€” and deliberately grid-local, because a station chosen when the script
...
const dockAtNearest: MacroDecider = (_step, obs, mem) => {
  ...
  if (obs.inWarp === true) {
    return tick(WAIT, "In warp â€” nothing decided mid-warp.", "Docking", ACTING, false, mem);
  }
  const snapshot = obs.snapshot ?? null;
  if (obs.inSpace !== true || snapshot === null) {
...
// â”€â”€ remote-cap â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
const remoteCap: MacroDecider = (_step, obs, mem) => {
  if (obs.inWarp === true) {
    return tick(WAIT, "In warp â€” nothing decided mid-warp.", "Feeding cap", ACTING, false, mem);
  }
  const snapshot = obs.snapshot ?? null;
  if (obs.inSpace !== true || snapshot === null) {
  ...
  if (!locked) {
    ...
    if (waited > MAX_LOCK_WAIT_TICKS) {
      return tick(WAIT, "That fleet-mate would not lock â€” watching for another.", "Feeding cap", ACTING, true, { ...mem, capLockOn: null });
    }
  ...
  if (outOfReach && num(mem, "capApproached") !== target.itemID) {
    return tick(
      { kind: "approach", targetID: target.itemID },
      "Closing in â€” too far out to pass them cap.",
      "Feeding cap",
      ACTING,
      true,
  ...
  const active = new Set(snapshot.ship?.activeModuleIDs ?? []);
  // Consecutive failures only â€” see the note in repHurtMate.
...
// â•â•â• Cargo extras â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•

// â”€â”€ jettison-cargo â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
// Dump the cargo hold into space as a can â€” the jetcan a miner fills instead of
...
const jettisonCargo: MacroDecider = (step, obs, mem) => {
  ...
  if (obs.flightStatus?.docked === true || obs.inSpace === false) {
    return tick(WAIT, "Not in space â€” a can has to go somewhere.", "Jettisoning", {
      kind: "blocked",
      reason: "Undock first â€” jettisoning drops a container into space.",
    });
  }
  if (obs.inSpace !== true) {
...
// â”€â”€ jettison-ore â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
// The same jetcan-drop as jettison-cargo, but empties the ORE hold (or
// whichever specialty hold â€” ice, gas â€” the ship carries) instead: a mining
...
const jettisonOre: MacroDecider = (step, obs, mem) => {
  ...
  // The specialty hold â€” ore, ice, gas, whichever this hull has â€” never cargo,
...
// â”€â”€ tidy-hangar â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
// Stack everything loose in the station hangar. One shot: the server merges what
// it can, and a second pass would find nothing to do â€” so this issues once and is
...
const tidyHangar: MacroDecider = (_step, obs, mem) => {
  if (obs.flightStatus?.docked !== true) {
    return tick(WAIT, "Not docked â€” the hangar is at a station.", "Tidying", {
      kind: "blocked",
      reason: "Dock at a station first â€” this block tidies its hangar.",
    });
  }
  if (flag(mem, "stacked")) {
...
// â”€â”€ compress-ore â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
// The fleet mechanic: a mining support ship on grid â€” your own hull or a
// fleet-mate's â€” running an industrial core plus compression gear is a FACILITY,
// and ore in your own hold can be squeezed to a fraction of its volume while you
// sit inside its range.
//
// âš  THE FACILITY IS FOUND FROM A READING, NOT GUESSED. `compressionFacility` is
...
// hopefully into the dark â€” which matters because the server answers "not a
...
//
// âš  AND EACH STACK IS JUDGED BY THE HOLD, NOT BY THE ANSWER. A compressed stack
...
// than retried forever â€” that is what keeps this bounded without pretending the
// refusal told us why.
...
const compressOre: MacroDecider = (_step, obs, mem) => {
  // âš  DOCKED is a verdict; UNREADABLE is not. `inSpace !== true` would lump the
  // two together and tell a player to undock a ship whose location simply had not
  // been read yet â€” the "null is never a verdict" rule, in the one place it is
  // easiest to get wrong.
  if (obs.flightStatus?.docked === true || obs.inSpace === false) {
    return tick(WAIT, "Not in space â€” compression happens at a ship on grid.", "Compressing", {
      kind: "blocked",
      reason: "Undock first â€” this block uses a support ship out on the grid.",
    });
  }
  if (obs.inSpace !== true) {
  ...
  if (obs.inWarp === true) {
    return tick(WAIT, "In warp â€” nothing decided mid-warp.", "Compressing", ACTING, false, mem);
  }
  const snapshot = obs.snapshot ?? null;
  const holds = obs.holds ?? null;
  ...
  // âš  A hold whose read FAILED carries items:null, which is "we could not look",
  // not "it is empty" â€” so a failed read must not finish the block. Wait for a
  ...
  if (facility === null) {
    return tick(WAIT, "No support ship is compressing here.", "Compressing", {
      kind: "blocked",
      reason:
        "No mining support ship on this grid is running its compression gear â€” bring one, or switch it on, and try again.",
    });
  }
  ...
  if (!isOwnShip) {
    ...
    if (distance !== null && distance > reach) {
      // Close in on it â€” the same shared ladder every other block uses. Its own
...
/** How many attempted stacks to remember â€” a hold cannot hold more than this. */
const MAX_TRIED_STACKS = 200;


// ── deploy-mtu-and-loot ──────────────────────────────────────────────────────
// One industrial-support sentence: while stationary in an Orca / Porpoise /
// Rorqual, deploy a Mobile Tractor Unit from cargo if none is on grid, then
// loot that MTU and any jetcans ALREADY within loot range. Deliberately does
// not approach distant cans — the support ship stays parked.
const deployMtuAndLoot: MacroDecider = (_step, obs, mem) => {
  if (obs.flightStatus?.docked === true || obs.inSpace === false) {
    return tick(WAIT, "Not in space — an MTU has to go somewhere.", "MTU support", {
      kind: "blocked",
      reason: "Undock first — this block deploys and loots in space.",
    });
  }
  if (obs.inSpace !== true) {
    return tick(WAIT, "Waiting for the ship to say where it is.", "MTU support", ACTING, false, mem);
  }

  const snapshot = obs.snapshot ?? null;
  if (snapshot === null) {
    return tick(WAIT, "Reading the grid.", "MTU support", ACTING, false, mem);
  }

  const ship = snapshot.ship ?? null;
  if (!isIndustrialSupportShip(ship)) {
    return tick(WAIT, "This hull is not an industrial support ship.", "MTU support", {
      kind: "blocked",
      reason:
        "Fly an Orca, Porpoise, or Rorqual — this block is for industrial mining support ships.",
    });
  }

  const parked = stationaryForIndustrialSupport(obs);
  if (parked.ok === null) {
    return tick(WAIT, parked.reason, "MTU support", ACTING, false, mem);
  }
  if (parked.ok === false) {
    return tick(WAIT, parked.reason, "MTU support", {
      kind: "blocked",
  ...
  const measurement = measureSpace(snapshot);
  const mtu = pickMtuOnGrid(snapshot, obs.myCharacterID ?? null);
  const lootedMtuRaw = mem["lootedMtuIDs"];
  const lootedMtu = new Set<number>(Array.isArray(lootedMtuRaw) ? (lootedMtuRaw as number[]) : []);
  const skippedCanRaw = mem["skippedCans"];
  const skippedCans = new Set<number>(Array.isArray(skippedCanRaw) ? (skippedCanRaw as number[]) : []);

  // 1) Deploy if nothing is on grid yet.
  if (mtu === null) {
    const cargo = obs.cargo ?? null;
    if (cargo === null) {
      return tick(WAIT, "Reading the cargo hold for a Mobile Tractor Unit.", "MTU support", ACTING, false, mem);
    }
    const aboard = mtusInCargo(cargo);
    if (aboard.length === 0) {
      return tick(WAIT, "No Mobile Tractor Unit to deploy.", "MTU support", {
        kind: "blocked",
        reason:
          "No Mobile Tractor Unit is on this grid or in cargo — load one, then try again.",
      });
    }
    const tries = (num(mem, "deployTries") ?? 0) + 1;
    if (tries > MAX_BLOCK_ATTEMPTS) {
      return tick(WAIT, "The Mobile Tractor Unit would not deploy.", "MTU support", {
        kind: "blocked",
        reason: "The Mobile Tractor Unit would not deploy after several tries, so the bot stopped.",
      });
    }
    return tick(
      { kind: "launchFromShip", itemIDs: [aboard[0]!.itemID] },
      "Deploying a Mobile Tractor Unit.",
      "MTU support",
      ACTING,
      true,
      { ...mem, deployTries: tries },
    );
  }

  // 2) Loot the MTU once it is in range (stay put — no approach).
  if (!lootedMtu.has(mtu.itemID)) {
    const dist = measurement?.distances.get(mtu.itemID) ?? null;
    if (dist === null) {
      return tick(WAIT, "Measuring range to the Mobile Tractor Unit.", "MTU support", ACTING, true, mem);
    }
    if (dist > INDUSTRIAL_LOOT_RANGE_M) {
      // Stationary contract: do not fly to it. Skip MTU loot and continue to in-range cans.
      return tick(WAIT, "Mobile Tractor Unit is out of loot range while parked.", "MTU support", ACTING, true, {
        ...mem,
        lootedMtuIDs: [...lootedMtu, mtu.itemID],
      });
    }
    const triesRaw = mem["mtuTries"];
    const mtuTries =
      triesRaw !== null && typeof triesRaw === "object" ? { ...(triesRaw as Record<string, number>) } : {};
    const targetTries = (mtuTries[mtu.itemID] ?? 0) + 1;
    if (targetTries > MAX_BLOCK_ATTEMPTS) {
      return tick(WAIT, "The Mobile Tractor Unit would not give up its contents.", "MTU support", ACTING, true, {
        ...mem,
        lootedMtuIDs: [...lootedMtu, mtu.itemID],
      });
    }
    // After the first successful issue we still re-try a few times; once the
    // attempt budget for this unit is spent we move on (MTUs do not despawn empty).
    if (targetTries > 1 && (num(mem, "mtuLootIssued") ?? 0) >= 1) {
      // Give the transfer one confirm tick, then treat as done for this unit.
      return tick(WAIT, "Moving on after looting the Mobile Tractor Unit.", "MTU support", ACTING, true, {
        ...mem,
        lootedMtuIDs: [...lootedMtu, mtu.itemID],
        mtuTries: { ...mtuTries, [mtu.itemID]: targetTries },
      });
    }
    mtuTries[mtu.itemID] = targetTries;
    return tick(
      { kind: "lootContainer", containerID: mtu.itemID },
      "Looting the Mobile Tractor Unit.",
      "MTU support",
      ACTING,
      true,
      { ...mem, mtuTries, mtuLootIssued: 1 },
    );
  }

  // 3) Loot jetcans already within range — never approach.
  const cans = jetcansWithinRange(snapshot, measurement, INDUSTRIAL_LOOT_RANGE_M).filter(
    (c) => !skippedCans.has(c.itemID),
  );
  if (cans.length === 0) {
    return tick(WAIT, "Mobile Tractor Unit handled; no jetcans left in range.", "MTU support", {
      kind: "done",
    });
  }
  const target = nearest(cans, measurement);
  ...
  const canTriesRaw = mem["canTries"];
  const canTries =
    canTriesRaw !== null && typeof canTriesRaw === "object"
      ? { ...(canTriesRaw as Record<string, number>) }
      : {};
  const targetTries = (canTries[target.itemID] ?? 0) + 1;
  if (targetTries > MAX_BLOCK_ATTEMPTS) {
    return tick(WAIT, "That jetcan would not give up its contents.", "MTU support", ACTING, true, {
      ...mem,
      skippedCans: [...skippedCans, target.itemID],
    });
  }
  canTries[target.itemID] = targetTries;
  return tick(
    { kind: "lootContainer", containerID: target.itemID },
    "Looting a jetcan within range.",
    "MTU support",
    ACTING,
    true,
    { ...mem, canTries },
...
// Warp to the nearest belt, then WAIT WHILE mineable asteroid count > X.
// Semantics (scriptText / catalog): done when count drops to X or below.
// Absent greaterThan => X = 0 (wait while any rocks remain).
const waitWhileAsteroidsAbove: MacroDecider = (step, obs, mem) => {
  const gateArg = step.args["greaterThan"];
  const greaterThan = gateArg !== undefined && gateArg.kind === "count" ? gateArg.value : 0;
  ...
  const snapshot = obs.snapshot ?? null;
  if (snapshot === null) {
    return tick(WAIT, "Reading the grid.", "Getting ready", ACTING, false, mem);
  }

  const measurement = measureSpace(snapshot);
  const belts = beltsOnGrid(snapshot);
  const rocks = snapshot.entities.filter((entity) => isMineableRock(entity));
  const arrived = flag(mem, "arrived");

  if (!arrived) {
    const belt = pickNextBelt(belts, measurement, "nearest");
    if (belt === null) {
      return tick(WAIT, "No asteroid belt in view.", "Nothing to fly to", {
        kind: "blocked",
        reason: "There is no asteroid belt here to warp to.",
      });
    }
    const beltDist = measurement?.distances.get(belt.itemID) ?? Number.POSITIVE_INFINITY;
    if (beltDist > BELT_ARRIVAL_RADIUS_M) {
      return tick(
        { kind: "warp", targetID: belt.itemID },
        `Warping to ${rockLabel(belt)}.`,
        "Flying to the belt",
        ACTING,
        false,
        mem,
    ...
    // Arrived — fall through to the count gate with arrived latched.
  }
  ...
  if (count > greaterThan) {
    return tick(
      WAIT,
      `At the belt — ${count} asteroid${count === 1 ? "" : "s"} (waiting while greater than ${greaterThan}).`,
      "Waiting on asteroids",
      ACTING,
      true,
      { ...mem, arrived: true },
    );
  ...
  return tick(
    WAIT,
    `Asteroid count is ${count} (no longer greater than ${greaterThan}).`,
    "Gate cleared",
    { kind: "done" },
  );
};
...
// Gate: only proceeds when no mineable asteroids remain. Then: note MTU scoop
// gap (no ship.ScoopDeployable API), recall drones, warp to nearest|random belt.
// Done once that warp has landed.
const emptyBeltCleanupNext: MacroDecider = (step, obs, mem) => {
  const modeArg = step.args["mode"];
  const mode =
    modeArg !== undefined && modeArg.kind === "beltTravel" && modeArg.mode === "random"
      ? "random"
      : "nearest";

  if (obs.inWarp === true) {
    return tick(WAIT, "In warp — nothing decided mid-warp.", "Leaving the belt", ACTING, false, mem);
  }
  if (obs.inSpace !== true) {
    return tick(WAIT, "Waiting for the ship to be out in space.", "Getting ready", ACTING, false, mem);
  }
  ...
  const measurement = measureSpace(snapshot);
  const targetID = num(mem, "nextBeltID");

  // Already chose a destination — wait until we are there, then done.
  if (targetID !== null) {
    const target = snapshot.entities.find((e) => e.itemID === targetID) ?? null;
    if (target === null) {
      return tick(WAIT, "Looking for the next belt on grid.", "Flying to the next belt", ACTING, false, mem);
    }
    const dist = measurement?.distances.get(targetID) ?? Number.POSITIVE_INFINITY;
    if (dist <= BELT_ARRIVAL_RADIUS_M) {
      return tick(WAIT, "At the next belt.", "Arrived", { kind: "done" });
    }
    const recalling = recallBeforeLeaving(obs, mem, "Recalling drones", targetID);
    if (recalling !== null) {
      return recalling;
    }
    return tick(
      { kind: "warp", targetID },
      `Warping to ${rockLabel(target)}.`,
  ...
  if (rocks.length > 0) {
    return tick(
      WAIT,
      `Belt still has ${rocks.length} asteroid${rocks.length === 1 ? "" : "s"} — waiting until empty.`,
      "Waiting for empty belt",
      ACTING,
      true,
      mem,
    );
  ...
  // Honest gap: no scoop-deployable API (only ScoopDrone / LaunchFromShip).
  const mtus = mtusOnGrid(snapshot);
  if (mtus.length > 0 && !flag(mem, "mtuScoopNoted")) {
    return tick(
      WAIT,
      "Mobile Tractor Units are on grid, but scooping deployables is not available yet — leaving them and continuing.",
      "Cleanup",
      ACTING,
      true,
      { ...mem, mtuScoopNoted: true },
    );
  }

  const belts = beltsOnGrid(snapshot);
  const exclude = new Set<number>();
  const here = pickNextBelt(belts, measurement, "nearest");
  if (here !== null && (measurement?.distances.get(here.itemID) ?? Number.POSITIVE_INFINITY) <= BELT_ARRIVAL_RADIUS_M) {
    exclude.add(here.itemID);
  }
  const next = pickNextBelt(belts, measurement, mode, exclude);
  if (next === null) {
    return tick(WAIT, "No other asteroid belt to warp to.", "Nothing to fly to", {
      kind: "blocked",
      reason: "No other belt is available to relocate to.",
    });
  }

  const recalling = recallBeforeLeaving(obs, mem, "Recalling drones", next.itemID);
  if (recalling !== null) {
    return { ...recalling, nextMem: { ...recalling.nextMem, nextBeltID: next.itemID } };
  }

  return tick(
    { kind: "warp", targetID: next.itemID },
    `Belt empty — warping to ${mode === "random" ? "a random" : "the nearest"} other belt (${rockLabel(next)}).`,
    "Flying to the next belt",
    ACTING,
    false,
    { ...mem, nextBeltID: next.itemID, recalled: false, aligned: false, recallWaited: 0 },
  );
};


const TRACTOR_ACTIVATE_RANGE_M = 20_000;

const tractorLootJetcans: MacroDecider = (_step, obs, mem) => {
  if (obs.flightStatus?.docked === true || obs.inSpace === false) {
    return tick(WAIT, "Not in space — nowhere to tractor a can.", "Tractor loot", {
      kind: "blocked",
      reason: "Undock first — this block tractors and loots jetcans in space.",
    });
  }
  if (obs.inSpace !== true) {
    return tick(WAIT, "Waiting for the ship to say where it is.", "Tractor loot", ACTING, false, mem);
  }
  const snapshot = obs.snapshot ?? null;
  if (snapshot === null) {
    return tick(WAIT, "Reading the grid.", "Tractor loot", ACTING, false, mem);
  }

  const tractors = obs.tractorModuleIDs ?? [];
  if (tractors.length === 0) {
    return tick(WAIT, "No tractor beams fitted.", "Tractor loot", {
      kind: "blocked",
      reason: "Fit online tractor beams — this block does not deploy a Mobile Tractor Unit.",
    });
  }

  const measurement = measureSpace(snapshot);
  const skippedRaw = mem["skippedCans"];
  const skipped = new Set<number>(Array.isArray(skippedRaw) ? (skippedRaw as number[]) : []);
  const cans = containersOnGrid(snapshot).filter((c) => !skipped.has(c.itemID));
  if (cans.length === 0) {
    return tick(WAIT, "No jetcans left on grid.", "Tractor loot", { kind: "done" });
  }
  ...
  if (dist === null) {
    return tick(WAIT, "Measuring range to a jetcan.", "Tractor loot", ACTING, true, mem);
  }

  // In loot range → loot (ore hold routing is in flow.transferLootedRows).
  if (dist <= INDUSTRIAL_LOOT_RANGE_M) {
    const tries = (num(mem, "lootTries") ?? 0) + 1;
    if (tries > MAX_BLOCK_ATTEMPTS) {
      return tick(WAIT, "That jetcan would not give up its contents.", "Tractor loot", ACTING, true, {
        ...mem,
        skippedCans: [...skipped, target.itemID],
        lootTries: 0,
      });
    }
    return tick(
      { kind: "lootContainer", containerID: target.itemID },
      "Looting a jetcan into the ore hold.",
      "Tractor loot",
      ACTING,
      true,
      { ...mem, lootTries: tries },
  ...
  // Lock + activate tractor beams on the can.
  const locked = new Set(obs.lockedTargetIDs ?? []);
  if (!locked.has(target.itemID)) {
    return tick(
      { kind: "lock", targetID: target.itemID },
      "Locking a jetcan for the tractor beams.",
      "Tractor loot",
      ACTING,
      true,
      mem,
  ...
  const activeIDs = snapshot.ship?.activeModuleIDs ?? null;
  if (activeIDs === null) {
    return tick(WAIT, "Waiting to see which modules are running.", "Tractor loot", ACTING, true, mem);
  }
  const active = new Set(activeIDs);
  const idleTractor = tractors.find((id) => !active.has(id));
  if (idleTractor !== undefined && dist <= TRACTOR_ACTIVATE_RANGE_M) {
    return tick(
      { kind: "activate", moduleID: idleTractor, targetID: target.itemID },
      "Activating a tractor beam on the jetcan.",
      "Tractor loot",
      ACTING,
      true,
      mem,
  ...
  // Approach until in loot range (tractors keep pulling once active).
  return tick(
    { kind: "approach", targetID: target.itemID },
    "Approaching the jetcan into loot range.",
    "Tractor loot",
    ACTING,
    true,
    mem,
...
// Miner fleet block: wait until the belt is empty, recall drones, fly to the
// bot home / starting structure (Upwell-aware via dockedLocationID + homeStationID),
// undock so an Upwell tether can engage, then hold idle (sustained).
// There is no separate tether API — Upwell tethering is automatic while stopped
// near a structure you can dock at after undock.
const emptyBeltHomeTetherWait: MacroDecider = (_step, obs, mem) => {
  if (obs.inWarp === true) {
    return tick(WAIT, "In warp — nothing decided mid-warp.", "Heading home", ACTING, false, mem);
  }

  const home = obs.homeStationID ?? obs.startingStationID ?? null;

  // Phase: already undocked at home — hold (tethered on Upwell; idle elsewhere).
  if (flag(mem, "holdingAtHome") === true) {
    if (obs.flightStatus?.docked === true) {
      return tick({ kind: "undock" }, "Undocking again to stay tethered at home.", "Tethering", ACTING, true, mem);
    }
    return tick(
      WAIT,
      "At home — tethered/holding until the next step or the fleet call.",
      "Holding at home",
      ACTING,
      true,
      mem,
  ...
  // Phase: docked at home — undock for tether, then latch hold.
  if (home !== null && obs.flightStatus?.docked === true && dockedLocationID(obs.flightStatus) === home) {
    return tick(
      { kind: "undock" },
      "Undocking at home so the structure tether can engage.",
      "Tethering",
      ACTING,
      false,
      { ...mem, holdingAtHome: true, recalled: false, aligned: false, recallWaited: 0 },
    );
  }

  // Still on a belt with rocks — wait for empty (unless we already left).
  if (!flag(mem, "beltCleared")) {
    if (obs.inSpace !== true) {
      return tick(WAIT, "Waiting for the ship to be out in space.", "Waiting for empty belt", ACTING, false, mem);
    }
    const snapshot = obs.snapshot ?? null;
    if (snapshot === null) {
      return tick(WAIT, "Reading the grid.", "Waiting for empty belt", ACTING, false, mem);
    }
    const rocks = snapshot.entities.filter((entity) => isMineableRock(entity));
    if (rocks.length > 0) {
      return tick(
    ...
    mem = { ...mem, beltCleared: true };
  }

  if (home === null) {
    return tick(WAIT, "Home is not known, so the miner cannot return.", "Heading home", {
      kind: "blocked",
      reason: "This bot does not know which station or structure is home (set bot home / start docked).",
    });
  }

  // Travel home (dock). Recall drones before leaving the belt.
  if (obs.flightStatus?.docked !== true || dockedLocationID(obs.flightStatus) !== home) {
    const onGrid = (obs.snapshot?.entities ?? []).some((e) => e.itemID === home);
    const recall = recallBeforeLeaving(obs, mem, "Heading home", onGrid ? home : null);
    if (recall !== null) {
      return { ...recall, nextMem: { ...recall.nextMem, beltCleared: true } };
    }
    const ride = rideAutopilotTo(obs, home, "Heading home");
    if (ride !== null) {
      return { ...ride, nextMem: { ...ride.nextMem, beltCleared: true, recalled: false, aligned: false, recallWaited: 0 } };
    }
    // Autopilot thinks we arrived but we are not docked yet — wait a tick.
    return tick(WAIT, "Arriving at home.", "Heading home", ACTING, false, { ...mem, beltCleared: true });
  }

  // Docked at home — fall through to undock on the next evaluation path above.
  return tick(
    { kind: "undock" },
    "Undocking at home so the structure tether can engage.",
    "Tethering",
    ACTING,
    false,
    { ...mem, beltCleared: true, holdingAtHome: true },
  );
};

// ── fleet-warp-to-me ─────────────────────────────────────────────────────────
// Boss block: optional asteroid-count gate (compose after wait-while-asteroids-above
// OR set greaterThan here), then SendBroadcast at the boss ship.
// LIMITATION: EveJS has no WarpFleet / warp-fleet-to-member bridge method. This
// issues a fleet location broadcast (InPosition) so members can warp to the ping;
// it does not force fleet members into warp.
const fleetWarpToMe: MacroDecider = (step, obs, mem) => {
  const gateArg = step.args["greaterThan"];
  const greaterThan = gateArg !== undefined && gateArg.kind === "count" ? gateArg.value : null;

  if (obs.inWarp === true) {
    return tick(WAIT, "In warp — nothing decided mid-warp.", "Fleet warp broadcast", ACTING, false, mem);
  }
  if (obs.inSpace !== true) {
    return tick(WAIT, "Waiting for the ship to be out in space.", "Fleet warp broadcast", ACTING, false, mem);
  }

  const inFleet = obs.inFleet ?? null;
  if (inFleet === null) {
    return tick(WAIT, "Checking your fleet status.", "Fleet warp broadcast", ACTING, false, mem);
  }
  if (inFleet === false) {
    return tick(WAIT, "You are not in a fleet.", "Fleet warp broadcast", {
      kind: "blocked",
      reason: "Form or join a fleet first — this block broadcasts your position to fleet members.",
    });
  }

  const snapshot = obs.snapshot ?? null;
  if (snapshot === null) {
    return tick(WAIT, "Reading the grid.", "Fleet warp broadcast", ACTING, false, mem);
  }

  if (greaterThan !== null) {
    const rocks = snapshot.entities.filter((entity) => isMineableRock(entity));
    if (rocks.length <= greaterThan) {
      return tick(
        WAIT,
        `Asteroid count is ${rocks.length} (need greater than ${greaterThan} before broadcasting).`,
        "Waiting on asteroids",
        ACTING,
        true,
        mem,
  ...
  if (flag(mem, "broadcastSent")) {
    return tick(
      WAIT,
      "Fleet location broadcast sent (members may warp to the ping — not a forced fleet warp).",
      "Broadcast sent",
      { kind: "done" },
    );
  }
  ...
  const shipID = snapshot.ship?.itemID ?? null;
  const typeID = snapshot.ship?.typeID ?? null;
  if (shipID === null) {
    return tick(WAIT, "Waiting to see your ship on the grid.", "Fleet warp broadcast", ACTING, false, mem);
  }

  const tries = (num(mem, "tries") ?? 0) + 1;
  if (tries > MAX_BLOCK_ATTEMPTS) {
    return tick(WAIT, "The fleet broadcast would not send.", "Fleet warp broadcast", {
      kind: "blocked",
      reason: "The fleet location broadcast failed after several tries, so the bot stopped.",
    });
  }
  ...
  return tick(
    ...
    "Broadcasting your position to the fleet (location ping — not a forced WarpFleet).",
    "Broadcasting warp-to-me",
    ACTING,
    false,
    { ...mem, tries, broadcastSent: true },
  );
};
...
export const SCRIPT_MACROS: CompleteMacroRegistry = {
  ...
  "deliver-ready-jobs": deliverReadyJobs,
  ...
  "join-fleet-from-advert": joinFleetFromAdvert,
  "standing-fleet-boss": standingFleetBoss,
  ...
  "deploy-mtu-and-loot": deployMtuAndLoot,
  "wait-while-asteroids-above": waitWhileAsteroidsAbove,
  "empty-belt-cleanup-next": emptyBeltCleanupNext,
  "tractor-loot-jetcans": tractorLootJetcans,
  "empty-belt-home-tether-wait": emptyBeltHomeTetherWait,
  "fleet-warp-to-me": fleetWarpToMe,
 ...
 * document's configured home every tick â€” fixed id, starting station, or a
 * station published onto the run board â€” and puts the result on the observation.
 * The shared autopilot makes that station reachable from any system.
 */
export const scriptTravelHome: HomeTravelDecider = (obs, mem) => {
  if (obs.flightStatus?.docked === true) {
    // Docked ANYWHERE is safe â€” the point of a fired watch is to be in a
