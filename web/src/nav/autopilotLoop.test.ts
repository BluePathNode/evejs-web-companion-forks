...
test("decide: docked at Upwell structure destination (structureID) -> arrived", () => {
  const structurePlan = {
    ...COMPILED,
    destinationStationID: 1030000000001,
  };
  assert.equal(
    decideAutopilotAction(
      status({
        docked: true,
        solarSystemID: DEST_SYSTEM,
        stationID: null,
    ...
    "arrived",
    "structureID matching the dock target is arrival",
  );
  assert.equal(
    decideAutopilotAction(
      status({
        ...
        solarSystemID: DEST_SYSTEM,
        stationID: null,
        structureID: 1030000000999,
      }),
      structurePlan,
      { warpedInSystem: null, jumpedFromSystem: null, pendingApproachGate: null },
    ).kind,
    "undock",
    "a different structure is not arrival",
  );
});
