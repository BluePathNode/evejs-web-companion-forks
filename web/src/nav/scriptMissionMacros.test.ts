...
test("deliver-ready-jobs: delivers only READY jobs, one per tick; confirms by re-read", () => {
  const deliver = SCRIPT_MACROS["deliver-ready-jobs"]!;
  const s = step("deliver-ready-jobs" as never);
  const jobs = [
    { jobID: 101, status: "ready" },
    { jobID: 102, status: "running" },
    { jobID: 103, status: "ready" },
    { jobID: 104, status: "delivered" },
  ];

  const first = deliver(s, obs({ industryJobs: jobs } as never), {}, NB);
  assert.ok(first.action.kind === "deliverIndustryJob" && first.action.jobID === 101);
  assert.equal(first.outcome.kind, "acting");

  // After 101 is gone from ready, next ready job is 103.
  const remaining = [
    { jobID: 102, status: "running" },
    { jobID: 103, status: "ready" },
    { jobID: 104, status: "delivered" },
  ];
  const second = deliver(s, obs({ industryJobs: remaining } as never), first.nextMem, NB);
  assert.ok(second.action.kind === "deliverIndustryJob" && second.action.jobID === 103);

  // No ready jobs left -> done.
  const noneReady = [
    { jobID: 102, status: "running" },
    { jobID: 104, status: "delivered" },
  ];
  const done = deliver(s, obs({ industryJobs: noneReady } as never), second.nextMem, NB);
  assert.equal(done.outcome.kind, "done");

  // Empty list -> done.
  const empty = deliver(s, obs({ industryJobs: [] } as never), {}, NB);
  assert.equal(empty.outcome.kind, "done");

  // Unreadable -> wait (acting), never invent a deliver.
  const unread = deliver(s, obs({ industryJobs: null } as never), {}, NB);
