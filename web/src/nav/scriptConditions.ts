...
export interface ScriptObservation {
  ...
  /**
   * Whether the session fleet currently has a live fleet-finder advert.
   * Read for standing-fleet-boss. null = unreadable.
   */
  readonly fleetAdvertLive?: boolean | null;
  /** Live advert summary for standing-fleet-boss match checks. null = none/unreadable. */
  readonly fleetAdvert?: {
  ...
  /** Pending fleet-finder join requests (boss side). null = unreadable. */
  readonly fleetJoinRequests?: readonly {
    readonly charID: number | null;
    readonly corpID: number | null;
    readonly allianceID: number | null;
  }[] | null;
  /**
   * Session-visible fleet-finder adverts (GetAvailableFleetAds) for join-fleet-from-advert.
   * null = unreadable; [] = none open to this character.
   */
  readonly availableFleetAds?: readonly {
  ...
  /** Fitted tractor beams (group name matches /tractor beam/i), for jetcan pull. */
  ...
  /**
   * The character's industry jobs, projected to what the deliver block needs:
   * each job's id and the SERVER's status string (including its own "ready"
   * derivation - the browser never clocks endDate itself). Read only when a
   * deliver-ready-jobs step is active. null = unreadable.
   */
  readonly industryJobs?: readonly {
