...
test("until-met latch: a fleeting unmet reading does NOT re-Activate mining lasers", () => {
  // Regression: until-met starts deactivating lasers; the next hold reading
  // flickers below the threshold (or goes null); without a latch, mine-at-belt
  // re-Activates and modules thrash on/off forever, never advancing.
  const s = script([macroStep("m", "mine-at-belt", { kind: "ore-hold-at-least", fraction: 0.9 }), macroStep("h", "deliver-ore")]);
  const activeSnapshot = { ship: { activeModuleIDs: [1] } } as unknown as ScriptObservation["snapshot"];
  const idleSnapshot = { ship: { activeModuleIDs: [] } } as unknown as ScriptObservation["snapshot"];
  let mem = initialMemory(s);
  const step = (o: ScriptObservation) => {
    const r = decideScriptAction(s, o, mem, registry, home);
    mem = r.memory;
    return r;
  };
  // until met, laser on -> deactivate and latch leaving
  const first = step(obs({ oreHoldFraction: 0.95, holdEmpty: false, miningModuleIDs: [1], snapshot: activeSnapshot }));
  assert.equal(first.action.kind, "deactivate");
  assert.equal(first.stepPath, "m");
  // until briefly looks unmet BUT laser still on -> must KEEP deactivating, never activate
  const mid = step(obs({ oreHoldFraction: 0.85, holdEmpty: false, miningModuleIDs: [1], snapshot: activeSnapshot }));
  assert.equal(mid.action.kind, "deactivate", "latched leaving must not re-Activate when until flickers");
  assert.notEqual(mid.action.kind, "activate");
  // lasers off, until still below — latch still forces leave (unlock/advance path)
  const doneLeaving = step(obs({
    oreHoldFraction: 0.85, holdEmpty: false, miningModuleIDs: [1], lockedTargetIDs: [],
    snapshot: idleSnapshot,
  }));
  assert.equal(doneLeaving.stepPath, "h", "latched leave advances once equipment is off");
});

test("repair thermostat OFF half needs recovery margin (no chatter at the trip line)", async () => {
  const { REPAIR_RECOVER_MARGIN } = await import("./scriptDecide.ts");
  const repairRow: InterruptRow = { id: "w", when: { kind: "shield-below", fraction: 0.5 }, respond: "repair" };
  const s = script([macroStep("m", "mine-at-belt", { kind: "ore-hold-at-least", fraction: 0.9 })], [repairRow, floor]);
  const runningShip = { ship: { activeModuleIDs: [700] } } as unknown as ScriptObservation["snapshot"];
  // Just above the trip threshold but inside the margin — must NOT shut off yet
  const chatter = decideScriptAction(
    s,
    obs({
      shieldRatio: 0.5 + REPAIR_RECOVER_MARGIN / 2,
      capacitorRatio: 0.8,
      shieldRepairerIDs: [700],
      snapshot: runningShip,
      oreHoldFraction: 0.4,
      holdEmpty: false,
      miningModuleIDs: [1],
    }),
  ...
  assert.notEqual(chatter.action.kind, "deactivate", "must not chatter-off at the trip line");
  // Clearly recovered — now shut off
  const healed = decideScriptAction(
    s,
    obs({
      shieldRatio: 0.5 + REPAIR_RECOVER_MARGIN + 0.05,
      capacitorRatio: 0.8,
      shieldRepairerIDs: [700],
      snapshot: runningShip,
      oreHoldFraction: 0.4,
  ...
  assert.equal(healed.action.kind, "deactivate");
});
