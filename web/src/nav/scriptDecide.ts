...
export type ScriptAction =
  ...
  /** Deliver ONE finished industry job (the retail CompleteJob - only ever gives). */
  | { readonly kind: "deliverIndustryJob"; readonly jobID: number }
  ...
  /** Post / refresh the session fleet's fleet-finder advert (server confirm-gated). */
  | { readonly kind: "postFleetAdvert"; readonly advertData: Readonly<Record<string, unknown>> }
  /** Apply to join a public fleet-finder advert (server confirm-gated). */
  | { readonly kind: "applyToJoinFleet"; readonly fleetID: number; readonly autoAccept: boolean }
  /** Fleet location broadcast (SendBroadcast) — ping, not forced WarpFleet. */
  | {
      readonly kind: "fleetBroadcast";
  ...
  /** Launch deployables (e.g. Mobile Tractor Unit) from ship cargo via ship.LaunchFromShip. */
  | { readonly kind: "launchFromShip"; readonly itemIDs: readonly number[] }
...
function equipmentToShutDownBeforeLeaving(step: MacroStep, obs: ScriptObservation): number | null {
  ...
  if (activeIDs === null) {
    return null; // unknown — caller waits rather than guessing
  }
...
/**
 * True when a repair watch's layer has recovered FAR ENOUGH past its trip
 * threshold that switching the repairers off will not immediately re-trip.
 * `not-met` alone is not enough — see REPAIR_RECOVER_MARGIN.
 */
function repairFullyRecovered(when: Condition, obs: ScriptObservation): boolean {
  const margin = REPAIR_RECOVER_MARGIN;
  switch (when.kind) {
    case "shield-below":
      return obs.shieldRatio !== null && obs.shieldRatio >= Math.min(1, when.fraction + margin);
    case "armor-below":
      return obs.armorRatio !== null && obs.armorRatio >= Math.min(1, when.fraction + margin);
    case "hull-below":
      return obs.hullRatio !== null && obs.hullRatio >= Math.min(1, when.fraction + margin);
    case "health-below":
      return obs.health !== null && obs.health >= Math.min(1, when.fraction + margin);
    default:
      // Non-layer watches never drive the repair thermostat's OFF half.
      return evaluateCondition(when, obs) === "not-met";
  }
}
...
function repairShutdown(
  ...
  // null activeModuleIDs is UNKNOWN — never "nothing running". Guessing here
  // either skips a needed shutdown or deactivates blindly.
  const activeIDs = obs.snapshot?.ship?.activeModuleIDs ?? null;
  ...
  for (const row of script.interrupts) {
    ...
    if (!repairFullyRecovered(row.when, obs)) {
      continue;
    }
...
function fireInterrupt(
  ...
  switch (row.respond) {
    ...
    case "repair": {
      ...
      // null = unknown module state — do not activate (may already be running)
      // and do not deactivate on a guess. Keep the program working this tick.
      const activeIDs = obs.snapshot?.ship?.activeModuleIDs ?? null;
      if (activeIDs === null) {
        return runProgram(script, obs, mem, registry, false);
      }
...
/**
 * How far ABOVE the watch threshold a layer must climb before the repair
 * thermostat switches the repairers back off.
 *
 * ⚠ WITHOUT THIS MARGIN the thermostat chatters. A shield-below-50% watch that
 * turns a booster ON at 0.49 and OFF the instant the layer reads 0.50 will, on
 * the next hit (or the next noisy read), turn it straight back ON — modules
 * thrash on/off every few ticks and the bot never gets back to mining. The
 * ON threshold stays the player's number; OFF requires a clear recovery.
 */
export const REPAIR_RECOVER_MARGIN = 0.2;
...
function runProgram(
  ...
  for (let hop = 0; hop <= maxHops; hop += 1) {
    ...
    if (step.until !== undefined) {
      ...
      // ⚠ LATCH THE LEAVING PATH. Once `until` has read met while armed, we
      // MUST keep shutting equipment down / unlocking / advancing even if the
      // next tick's hold reading flickers back below the threshold (or goes
      // unreadable). Without the latch, mine-at-belt re-Activates the lasers
      // the moment until briefly looks unmet — modules thrash on/off and the
      // step never actually leaves. The latch dies with the step's macroMem
      // when we finally advance (omit below).
      const alreadyLeaving = stepMem["untilLeaving"] === true;
      const leavingNow = alreadyLeaving || (verdict === "met" && tick.armed);
      if (leavingNow && tick.armed) {
        macroMem = {
          ...macroMem,
          [step.id]: { ...macroMem[step.id], untilLeaving: true },
        };
        // Unknown activeModuleIDs: wait rather than advance (might leave lasers
        // running) or deactivate on a guess.
        // Only wait on unknown actives when this ship actually has miners that
        // might still be cycling. An empty/missing miningModuleIDs list means
        // there is nothing to shut down — advance (tests and non-mining fits).
        const activeIDs = obs.snapshot?.ship?.activeModuleIDs ?? null;
        const minersFitted = (obs.miningModuleIDs ?? []).length > 0;
        if (activeIDs === null && step.macro === "mine-at-belt" && minersFitted) {
          return {
            action: { kind: "wait" },
            why: "The until condition is met — waiting to see whether the mining equipment is still running.",
            phase: tick.phase,
            stepPath: step.id,
            interruptID: null,
            status: "running",
