...
/**
 * The dockable location the ship is IN when docked: an NPC station id, or an
 * Upwell structure id. Flight status reports them on separate fields; bots and
 * arrival checks treat either as "home / destination dock".
 */
export function dockedLocationID(status: Pick<FlightStatus, "stationID" | "structureID">): number | null {
  return status.stationID ?? status.structureID;
}
